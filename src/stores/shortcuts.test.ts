...
describe("shortcuts store", () => {
  ...
  it("persists each platform independently and rejects a conflict without replacing either action", async () => {
    const store = useShortcutsStore();
    expect(await store.setBinding("macos", "terminal.search", "Meta+Shift+KeyG")).toEqual({ ok: true });
    ...
    expect(await store.setBinding("macos", "terminal.copy", "Meta+Shift+KeyG")).toMatchObject({
      ok: false,
      reason: "conflict",
  ...
  it("keeps the previous configuration when persistence rejects an update or import", async () => {
    const store = useShortcutsStore();
    ...
    expect(await store.setBinding("macos", "terminal.search", "Meta+Shift+KeyG")).toEqual({
      ok: false,
      reason: "storage-error",
    });
    expect(store.macosBindings["terminal.search"]).toBe(previous);
    expect(await store.importProfile(store.exportProfile())).toEqual({ ok: false, reason: "storage-error" });
