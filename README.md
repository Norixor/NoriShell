...
- **一个窗口完成远程工作。** 切换终端、主机、文件传输、隧道、服务器概览和远程桌面。
- **无需账号即可使用。** 主机配置留在本机，保存的密码和私钥进入加密 Vault。
- **连接互不干扰。** 关闭终端、文件传输、隧道或监控中的一项，不会中断其他连接。
- **按需扩展。** 通过本地 ZIP 安装或升级插件；支持 macOS Apple Silicon / Intel、Windows x64 / ARM64。
...
| 主机与连接管理 | 分组、标签、收藏、OpenSSH 配置导入、密码/密钥/Agent 登录和服务器指纹确认 |
| 连接路由 | 直连、HTTP CONNECT / SOCKS5 代理、跳板机和单主机算法设置 |
| 文件与网络 | 多窗格 SFTP 浏览、传输、预览/编辑、实时追踪，以及本地/远程/动态端口转发 |
| 远程桌面 | RDP / VNC 配置与会话、SSH 网关、缩放、输入、文本剪贴板和 RDP 音频能力 |
| 服务器概览 | 查看各主机的连接状态，以及可选的 CPU、内存、网络和磁盘监控 |
| 凭据保护 | 加密 Vault、临时凭据和服务器指纹确认 |
| 日常操作 | 快捷命令、自定义快捷键、关键词高亮、通知、托盘面板和设置导入导出 |
| 插件与主题 | 通过本地 ZIP 安装或升级插件，按需启用主题 |
| 可选同步 | 通过 Norixor 插件或自建服务同步可移植 SSH、远程桌面配置与非秘密偏好，不上传本机 Vault 文件 |
| Telnet | 独立 Telnet 会话，连接前提示明文传输风险 |
...
- 保存的密码和私钥存放在加密 Vault 中；首次 SSH 连接需要确认服务器指纹，指纹变化会阻断连接。
- 插件需要用户授权才能使用相应能力，不能读取 Vault；同步不会上传本机 Vault 文件。
...
| [Core API 目录](docs/guides/core-api/README.zh-CN.md) | 桌面应用内部命令、事件与类型参考 |
