                ...
                .contains(ironrdp_pdu::bitmap::Compression::BITMAP_COMPRESSION)
            {
                if update.bits_per_pixel == 32 {
                    ...
                    match self.bitmap_stream_decoder.decode_bitmap_stream_to_rgb24(
                        ...
                        Ok(()) => {
                            let pixels = bitmap_rows(&buf, update.width, update.height, 3, false, &update.rectangle)?;
                            image.apply_rgb24(&pixels, &update.rectangle, true)?
                        }
                        Err(err) => {
                    ...
                    match ironrdp_graphics::rle::decompress(
                        ...
                        Ok(RlePixelFormat::Rgb16) => {
                            let pixels = bitmap_rows(&buf, update.width, update.height, 2, false, &update.rectangle)?;
                            image.apply_rgb16_bitmap(&pixels, &update.rectangle)?
                        }
                        Ok(RlePixelFormat::Rgb15) => {
                            let pixels = bitmap_rows(&buf, update.width, update.height, 2, false, &update.rectangle)?;
                            image.apply_rgb15_bitmap(&pixels, &update.rectangle)?
                        }
                        Ok(RlePixelFormat::Rgb24) => {
                            let pixels = bitmap_rows(&buf, update.width, update.height, 3, false, &update.rectangle)?;
                            image.apply_bgr24_bitmap(&pixels, &update.rectangle)?
                        }
                        Ok(RlePixelFormat::Rgb8) => {
                            let pixels = bitmap_rows(&buf, update.width, update.height, 1, false, &update.rectangle)?;
                            image.apply_rgb8_with_palette(&pixels, &update.rectangle, self.palette.colors())?
                        }
                ...
                match update.bits_per_pixel {
                    8 => image.apply_rgb8_with_palette(&pixels, &update.rectangle, self.palette.colors())?,
                    15 => image.apply_rgb15_bitmap(&pixels, &update.rectangle)?,
                    16 => image.apply_rgb16_bitmap(&pixels, &update.rectangle)?,
                    24 => image.apply_bgr24_bitmap(&pixels, &update.rectangle)?,
                    32 => image.apply_rgb32_bitmap(&pixels, PixelFormat::BgrX32, &update.rectangle)?,
                    _ => {
...
/// Preserve the source scanline width before cropping to the destination. The
/// TS_BITMAP_DATA width can include right-hand pixels outside destRight (xrdp
/// aligns compressed bitmaps too). Image apply methods require tightly packed
/// destination rows; using their width as the source stride shifts every row.
fn bitmap_rows<'a>(
    data: &'a [u8],
    width: u16,
    height: u16,
    ...
    {
        return Err(reason_err!("Bitmap", "invalid source or destination dimensions"));
    }
    let visible_width = usize::from(rectangle.right) - usize::from(rectangle.left) + 1;
    let visible_height = usize::from(rectangle.bottom) - usize::from(rectangle.top) + 1;
    if visible_width > usize::from(width) || visible_height > usize::from(height) {
        return Err(reason_err!("Bitmap", "destination exceeds source bitmap"));
    }
    let row_bytes = usize::from(width) * bytes_per_pixel;
    let stride = if padded { (row_bytes + 3) & !3 } else { row_bytes };
    let expected = stride
        .checked_mul(usize::from(height))
        .ok_or_else(|| reason_err!("Bitmap", "source bitmap size overflow"))?;
    if data.len() != expected {
        return Err(reason_err!("Bitmap", "source bitmap byte length mismatch"));
    }
    let visible_bytes = visible_width * bytes_per_pixel;
    if stride == visible_bytes && visible_height == usize::from(height) {
        return Ok(Cow::Borrowed(data));
    }
    // Source rows are bottom-up. Preserve the top-left visible area, then let
    // the existing image method reverse the packed rows exactly once.
    let mut packed = Vec::with_capacity(visible_bytes * visible_height);
    for row in data.chunks_exact(stride).skip(usize::from(height) - visible_height) {
        packed.extend_from_slice(&row[..visible_bytes]);
    }
    Ok(Cow::Owned(packed))
}
...
mod bitmap_stride_tests {
    ...
    fn rectangle() -> InclusiveRectangle {
        InclusiveRectangle {
            left: 1,
            top: 1,
            right: 2,
    ...
    fn assert_pixels(image: &DecodedImage) {
        for y in 0..4 {
            for x in 0..4 {
                let expected: &[u8] = match (x, y) {
                    (1, 1) => &[255, 0, 0],
                    (2, 1) => &[0, 255, 0],
                    (1, 2) => &[0, 0, 255],
                    (2, 2) => &[255, 255, 255],
    ...
    #[test]
    fn planar_compressed_bitmap_preserves_source_stride_and_destination_bounds() {
        // Bottom-up 4x2 source, only the left 2x2 is visible. Distinct trailing
        // pixels make accidental destination-width slicing observable.
        let source = [
            0, 0, 255, 255, 255, 255, 90, 80, 70, 60, 50, 40, 255, 0, 0, 0, 255, 0, 30, 20, 10, 120, 110, 100,
        ];
        for rle in [false, true] {
            let mut encoded = vec![0; 1024];
            let len = BitmapStreamEncoder::new(4, 2)
                .encode_bitmap::<RgbChannels>(&source, &mut encoded, rle)
                .unwrap();
                ...
                .process_bitmap_update(
                    ...
                    BitmapUpdateData {
                        rectangles: vec![BitmapData {
                            ...
                            height: 2,
                            bits_per_pixel: 32,
                            compression_flags: Compression::BITMAP_COMPRESSION | Compression::NO_BITMAP_COMPRESSION_HDR,
                            compressed_data_header: None,
                            bitmap_data: &encoded[..len],
                        }],
                    },
                )
                .unwrap();
    ...
    #[test]
    fn interleaved_rle_bitmap_preserves_source_stride() {
        // REGULAR_COLOR_IMAGE, 8 literal BGR pixels in bottom-up order.
        let source = [
            0x88, 255, 0, 0, 255, 255, 255, 70, 80, 90, 40, 50, 60, 0, 0, 255, 0, 255, 0, 10, 20, 30, 100, 110, 120,
        ];
        let mut image = DecodedImage::new(PixelFormat::RgbA32, 4, 4);
        processor()
            .process_bitmap_update(
    ...
    #[test]
    fn raw_bitmap_strips_wire_padding_and_extra_source_pixels() {
        // 3 pixels * 3 bytes, rounded to a 12-byte wire row. The destination
        // has 2 pixels; neither the third source pixel nor byte padding is drawn.
        let source = [
            255, 0, 0, 255, 255, 255, 70, 80, 90, 3, 2, 1, 0, 0, 255, 0, 255, 0, 10, 20, 30, 6, 5, 4,
        ];
        let mut image = DecodedImage::new(PixelFormat::RgbA32, 4, 4);
        processor()
            .process_bitmap_update(
    ...
    #[test]
    fn invalid_source_extent_and_payload_fail_before_image_application() {
        assert!(bitmap_rows(&[0; 12], 1, 2, 3, false, &rectangle()).is_err());
        assert!(bitmap_rows(&[0; 23], 4, 2, 3, false, &rectangle()).is_err());
        assert!(bitmap_rows(&[0; 25], 4, 2, 3, false, &rectangle()).is_err());
        assert!(bitmap_rows(&[], 0, 0, 3, false, &rectangle()).is_err());
        let reversed = InclusiveRectangle {
            left: 2,
            top: 1,
            right: 1,
            bottom: 2,
        };
        assert!(bitmap_rows(&[0; 24], 4, 2, 3, false, &reversed).is_err());
    }
}
