...
use super::{error::Error, run_with_max_packet_len, runtime, Handler};
use crate::{
    client::Config,
    de,
    extensions::{
...
impl SessionInner {
    pub fn reply(&mut self, id: Option<u32>, packet: Packet) -> SftpResult<()> {
        ...
        // A request future may be cancelled after its packet was sent. Its
        // guard removes the pending entry immediately; a later reply for that
        // request is therefore expected and can be ignored by request id.
        Ok(())
    }
}
...
impl RawSftpSession {
    ...
    where
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        let maximum_packet_len = cfg.max_packet_len;
        ...
        Self {
            tx: run_with_max_packet_len(stream, inner, maximum_packet_len),
            requests: req_map,
    ...
    fn send_pending(&self, id: Option<u32>, packet: Packet) -> SftpResult<PendingResponse> {
        let receiver = self.send(id, packet)?;
        Ok(PendingResponse {
            receiver,
            _guard: PendingRequestGuard {
    ...
    async fn request(&self, id: Option<u32>, packet: Packet) -> SftpResult<Packet> {
        let pending = self.send_pending(id, packet)?;
        let timeout = self.timeout.load(Ordering::Relaxed);

        match runtime::timeout(Duration::from_secs(timeout), pending).await {
            Ok(Ok(result)) => result,
    ...
    pub(crate) fn close_nowait(&self, handle: String) -> SftpResult<PendingResponse> {
        let id = self.use_next_id();
        self.send_pending(Some(id), Close { id, handle }.into())
    }
...
mod tests {
    ...
    #[test]
    fn dropping_a_nowait_response_removes_its_pending_request() {
