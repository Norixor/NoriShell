...
pub use session::{DirectoryStream, SftpSession};
...
async fn process_handler<S, H>(
    stream: &mut S,
    handler: &mut H,
    maximum_packet_len: u32,
) -> Result<(), Error>
where
    S: AsyncRead + Unpin,
    H: Handler + Send,
{
    let mut bytes = read_packet(stream, maximum_packet_len).await?;
...
{
    run_with_max_packet_len(stream, handler, Config::default().max_packet_len)
}

pub(crate) fn run_with_max_packet_len<S, H>(
    stream: S,
    mut handler: H,
    maximum_packet_len: u32,
    ...
    H: Handler + Send + 'static,
{
    ...
    let wc = rc.clone();
    {
        runtime::spawn(async move {
            loop {
                select! {
                    result = process_handler(&mut rd, &mut handler, maximum_packet_len) => {
                        match result {
...
mod tests {
    ...
    #[tokio::test]
    async fn configured_packet_limit_rejects_frame_before_payload_allocation() {
        let (mut writer, mut reader) = tokio::io::duplex(16);
        writer.write_u32(17).await.unwrap();
        let result = process_handler(&mut reader, &mut NoopHandler, 16).await;
        assert!(matches!(
            result,
            Err(Error::BadMessage(message))
                if message.contains("packet length limit exceeded")
        ));
    }
}
