...
fn read_run_length(reader: &mut ZlibReader, maximum: usize) -> Result<usize, VncError> {
    let mut run_length: usize = 1;
    loop {
        let run_length_part = reader.read_u8()?;
...
fn copy_indexed(
    ...
    let end = start.checked_add(bpp).ok_or(VncError::InvalidImageData)?;
