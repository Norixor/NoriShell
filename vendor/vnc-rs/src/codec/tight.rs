...
impl Decoder {
        ...
        Fut: Future<Output = Result<(), VncError>>,
    {
        ...
        for i in 0..total {
            ...
            offset -= 1;
            let packed = *data.get(index as usize).ok_or(VncError::InvalidImageData)?;
            let sp = ((packed >> offset) & 0x01) as usize * 3;
            let color = self
                .palette
                .get(sp..sp + 3)
        ...
        Fut: Future<Output = Result<(), VncError>>,
    {
        ...
        let mut dp = 0;
        while i < total {
            let sp = *data.get(i).ok_or(VncError::InvalidImageData)? as usize * 3;
            let color = self
                .palette
                .get(sp..sp + 3)
                .ok_or(VncError::InvalidImageData)?;
