...
use crate::pdu::{
    CapabilitiesRequestPdu, CapsVersion, ClosePdu, CreateRequestPdu, CreationStatus,
    DrdynvcClientPdu, DrdynvcServerPdu, SoftSyncChannelList, SoftSyncRequestPdu,
    SoftSyncTunnelType,
};
use crate::{
    CompleteData, DvcProcessor, DynamicChannelMut, DynamicChannelRef, encode_dvc_messages,
};
...
impl DynamicChannelAllocator {
        ...
        T: DvcServerProcessor + 'static,
    {
        self.dynamic_channels.insert(
            channel_id,
            DynamicChannel::new(processor, channel_id, state),
        );
    }
...
impl DrdynvcServer {
        ...
        T: DvcServerProcessor + 'static,
    {
        let channel_id = self
            .dynamic_channels
            .insert_channel(channel, ChannelState::Pending);
        self.type_id_to_channel_id
            .insert(TypeId::of::<T>(), channel_id);
        self
    }
    ...
    pub fn dvc_by_id_mut<T: DvcServerProcessor>(
        &mut self,
        id: u32,
    ...
    pub fn request_reliable_udp(&mut self, channel_ids: Vec<u32>) -> PduResult<SvcMessage> {
        if channel_ids.is_empty() {
            return Err(pdu_other_err!(
                "soft-sync requires at least one dynamic channel"
            ));
        }
        ...
        for channel_id in &channel_ids {
            ...
            {
                return Err(pdu_other_err!(
                    "soft-sync channel list contains a duplicate channel ID"
                ));
            }
        }
    ...
    pub fn process_tunnel(&mut self, payload: &[u8]) -> PduResult<Vec<SvcMessage>> {
        ...
        if !self
            .incoming_tunnel_channels
            .contains_key(&data.channel_id())
        {
            return Err(pdu_other_err!(
    ...
    fn process_soft_sync_response(
        &mut self,
        response: crate::pdu::SoftSyncResponsePdu,
    ) -> PduResult<()> {
        ...
        for tunnel_type in response.tunnels_to_switch() {
            if !requested_tunnels.contains(tunnel_type) {
                return Err(pdu_other_err!(
                    "soft-sync response selected an unrequested tunnel"
                ));
            }
        }
...
mod tests {
    ...
    impl DvcProcessor for TestDvc {
        ...
        fn process(
            ...
            _channel_id: u32,
            _payload: &[u8],
        ) -> PduResult<Vec<crate::DvcMessage>> {
            Ok(Vec::new())
        }
    }
    ...
    fn soft_sync_rejects_tunnel_data_until_the_client_responds() {
        ...
        let channel_id = server
            .dynamic_channels
            .insert_channel(TestDvc, ChannelState::Opened);

        server
            .request_reliable_udp(alloc::vec![channel_id])
            .unwrap();
        assert_eq!(
        ...
        let tunnel_data = ironrdp_core::encode_vec(&DrdynvcClientPdu::Data(
            crate::pdu::DrdynvcDataPdu::Data(crate::pdu::DataPdu::new(channel_id, Vec::new())),
        ))
        .unwrap();
