...
use ironrdp_pdu::utils::{
    CharacterSet, checked_sum, encoded_str_len, read_string_from_cursor, strict_sum,
...
impl SoftSyncRequestPdu {
    ...
    const FIXED_PART_SIZE: usize =
        Header::FIXED_PART_SIZE + 1 /* Pad */ + Self::LENGTH_FIXED_PART_SIZE;
    ...
    fn decode(header: Header, src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ...
        let length = usize::try_from(src.read_u32())
            .map_err(|_| invalid_field_err!("Length", "is too large"))?;
        ...
        if flags & Self::TCP_FLUSHED == 0 {
            return Err(invalid_field_err!(
                "Flags",
                "SOFT_SYNC_TCP_FLUSHED must be set"
            ));
        }
    ...
    fn length(&self) -> usize {
        Self::LENGTH_FIXED_PART_SIZE
            + self
                .channel_lists
                .iter()
                .map(SoftSyncChannelList::size)
                .sum::<usize>()
    }
...
impl SoftSyncResponsePdu {
    ...
    fn decode(header: Header, src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ...
        let number_of_tunnels = usize::try_from(src.read_u32())
            .map_err(|_| invalid_field_err!("NumberOfTunnels", "is too large"))?;
...
impl DataFirstPdu {
    ...
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        self.header.sp.encode_val(
            cast_length!("DataFirstPdu::Length", self.length, in: dst)?,
            dst,
        )?;
