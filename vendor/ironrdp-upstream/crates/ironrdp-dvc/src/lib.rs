...
pub use ironrdp_pdu;
use ironrdp_core::{
    AsAny, Decode as _, Encode, EncodeResult, ReadCursor, assert_obj_safe, cast_length, encode_vec,
    other_err,
};
...
impl DvcMessageBatch {
    ...
    pub fn try_new(channel_id: DynamicChannelId, messages: Vec<SvcMessage>) -> PduResult<Self> {
        for message in &messages {
            ...
            let pdu = pdu::DrdynvcClientPdu::decode(&mut ReadCursor::new(&encoded))
            ...
            match pdu {
                ...
                pdu::DrdynvcClientPdu::Data(_) => {
                    return Err(pdu_other_err!(
                        "DVC message channel ID does not match its batch"
                    ));
                }
