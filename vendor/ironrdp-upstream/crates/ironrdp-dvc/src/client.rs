...
use crate::pdu::{
    ...
    DrdynvcClientPdu, DrdynvcDataPdu, DrdynvcServerPdu, SoftSyncResponsePdu, SoftSyncTunnelType,
};
use crate::{
    DvcMessage, DvcMessageBatch, DvcProcessor, DynamicChannelId, DynamicChannelMut,
    DynamicChannelName, DynamicChannelRef, encode_dvc_messages,
};
...
impl DrdynvcClient {
    ...
    pub fn attach_established_dynamic_channel<T>(
        &mut self,
        ...
        T: DvcClientProcessor,
    {
        let dvc_channel = self
            .dynamic_channels
            .get_by_type_id_mut(TypeId::of::<T>())?;
        let channel_id = dvc_channel.channel_id?;
        dvc_channel
            .channel_processor
            .as_any_mut()
    ...
    {
        self.dynamic_channels
            .has_listener_by_type_id(TypeId::of::<T>())
    }
    ...
    ///
    /// Returns `None` when the channel ID is unknown or the processor has a different type.
    pub fn get_dvc_by_channel_id_mut<T>(
        &mut self,
        channel_id: u32,
    ...
    fn create_capabilities_response(&mut self, server_version: CapsVersion) -> SvcMessage {
        let caps_response =
            DrdynvcClientPdu::Capabilities(CapabilitiesResponsePdu::new(server_version));
    ...
    pub fn close_channel(&mut self, channel_id: u32) -> Option<SvcMessage> {
        ...
        Some(SvcMessage::from(DrdynvcClientPdu::Close(ClosePdu::new(
            channel_id,
        ))))
    }
    ...
    pub fn has_channels_on_tunnel(&self, tunnel_type: SoftSyncTunnelType) -> bool {
        self.tunnel_channels
            .values()
            .any(|selected| *selected == tunnel_type)
    }
    ...
    fn process_soft_sync_request(
        &mut self,
        request: crate::pdu::SoftSyncRequestPdu,
        ...
        for list in request.channel_lists() {
            ...
            if !self.available_tunnels.contains(&list.tunnel_type()) {
                return Err(pdu_other_err!(
                    "soft-sync request selected an unavailable tunnel"
                ));
            }
        ...
        let response = SvcMessage::from(DrdynvcClientPdu::SoftSyncResponse(
    ...
    fn process(&mut self, payload: &[u8]) -> PduResult<Vec<SvcMessage>> {
        ...
        match pdu {
            ...
            DrdynvcServerPdu::Create(create_request) => {
                ...
                let (creation_status, start_messages) = if let Some(dvc) = self
                    .dynamic_channels
                    .try_create_channel(&channel_name, channel_id)
                {
            ...
            DrdynvcServerPdu::Close(close) => {
                ...
                if self
                    .dynamic_channels
                    .remove_by_channel_id(channel_id)
                    .is_some()
                {
            ...
            DrdynvcServerPdu::Data(data) => {
                if self.tunnel_channels.contains_key(&data.channel_id()) {
                    return Err(pdu_other_err!(
                        "received TCP data for a channel selected by Soft-Sync"
                    ));
                }
    ...
    fn remove_by_channel_id(&mut self, id: DynamicChannelId) -> Option<DynamicVirtualChannel> {
        self.active_channels.remove(&id).inspect(|dvc| {
            ...
            if let alloc::collections::btree_map::Entry::Occupied(entry) =
                self.type_id_to_channel_id.entry(type_id)
                && entry.get() == &id
            {
                entry.remove();
            }
...
mod tests {
    ...
    fn add_active_channel(client: &mut DrdynvcClient, channel_id: DynamicChannelId) {
        client.dynamic_channels.active_channels.insert(
            channel_id,
            DynamicVirtualChannel::from_boxed(Box::new(TestDvc)),
        );
    }
    ...
    fn message_batch_rejects_a_mismatched_channel_id() {
        let message = SvcMessage::from(DrdynvcClientPdu::Data(DrdynvcDataPdu::Data(
