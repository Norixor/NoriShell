...
impl<'a> BitStreamReader<'a> {
    ...
    ///
    /// Shifts the accumulator left and fills from the prefetch buffer.
    /// When crossing a 32-bit boundary, it advances the byte pointer
    /// and re-prefetches.
    #[expect(
        clippy::as_conversions,
        reason = "nbits (u32 ≤ 31) always fits in usize"
    )]
...
impl<'a> BitStreamWriter<'a> {
    ...
    #[expect(
        clippy::as_conversions,
        reason = "nbits (u32 ≤ 32) always fits in usize"
    )]
...
mod tests {
    ...
    fn roundtrip_multiple_values() {
        let mut buf = [0u8; 16];
        let (byte_len, total_bits) = write_and_flush(
            &mut buf,
            &[(0b110, 3), (0xFF, 8), (0b10101, 5), (0xCAFE, 16)],
        );
    ...
    fn roundtrip_across_boundary() {
        let mut buf = [0u8; 16];
        let (byte_len, _) = write_and_flush(
            &mut buf,
            &[(0x1234, 16), (0x5678, 16), (0x9ABC, 16), (0xDEF0, 16)],
        );
