//! MPPC constants and lookup tables.

/// RDP4 history buffer size (8 KB).
pub(crate) const HISTORY_BUFFER_SIZE_RDP4: usize = 8192;

/// RDP5 history buffer size (64 KB).
pub(crate) const HISTORY_BUFFER_SIZE_RDP5: usize = 65536;

/// Match buffer size used for compression hash table.
pub(crate) const MATCH_BUFFER_SIZE: usize = 32768;

/// History buffer wrapping mask for RDP4 (8K - 1 = 0x1FFF).
pub(crate) const HISTORY_MASK_RDP4: usize = 0x1FFF;

/// History buffer wrapping mask for RDP5 (64K - 1 = 0xFFFF).
pub(crate) const HISTORY_MASK_RDP5: usize = 0xFFFF;

/// Computes a 15-bit hash index from three consecutive bytes.
///
/// Equivalent to:
/// ```c
...
#[expect(
    ...
    reason = "result masked to 15 bits, always fits in usize"
)]
...
/// 256-entry lookup table used for 3-byte hash computation.
///
#[rustfmt::skip]
pub(crate) static MPPC_MATCH_TABLE: [u32; 256] = [
