...
use ironrdp_core::{
    AsAny, DecodeResult, Encode, EncodeResult, ReadCursor, WriteBuf, WriteCursor, assert_obj_safe,
    decode_cursor, encode_buf,
};
...
impl StaticVirtualChannel {
    ...
    fn chunkify_with_max_chunk_len(
        messages: Vec<SvcMessage>,
        max_chunk_len: usize,
    ) -> EncodeResult<Vec<WriteBuf>> {
...
impl ChunkProcessor {
    ...
    fn dechunkify(&mut self, payload: &[u8]) -> DecodeResult<Option<Vec<u8>>> {
        ...
        if !header
            .flags
            .contains(ChannelControlFlags::PACKET_COMPRESSED)
            && self.chunked_pdu.len() > expected_length
        {
            self.clear_sequence();
            return Err(ironrdp_core::invalid_field_err!(
                "length",
        ...
        if !header
            .flags
            .contains(ChannelControlFlags::PACKET_COMPRESSED)
            && self.chunked_pdu.len() != sequence_length
        {
            self.clear_sequence();
            return Err(ironrdp_core::invalid_field_err!(
                "length",
...
impl StaticChannelSet {
    ...
    pub fn insert_dynamic<T: SvcProcessor + 'static>(
        &mut self,
        val: T,
    ) -> Option<StaticChannelKey> {
    ...
    pub fn get_by_channel_name_key(
        &self,
        name: &ChannelName,
    ) -> Option<(StaticChannelKey, &StaticVirtualChannel)> {
        self.iter_by_key()
            .find(|(_, x)| x.channel_processor.channel_name() == *name)
    }
    ...
    pub fn attach_channel_id_by_key(
        ...
        if let Some(previous_channel_id) =
            previous_channel_id.filter(|previous| *previous != channel_id)
        {
    ...
    pub fn get_channel_id_by_type_id(&self, type_id: TypeId) -> Option<StaticChannelId> {
        self.to_channel_id
            .get(&StaticChannelKey::Typed(type_id))
            .copied()
    }
    ...
    pub fn iter_mut(
        ...
        self.channels
            .iter_mut()
            .filter_map(move |(key, svc)| match key {
                StaticChannelKey::Typed(type_id) => {
                    Some((*type_id, svc, to_channel_id.get(key).copied()))
                }
                StaticChannelKey::Dynamic(_) => None,
            })
    }
