...
impl<'de> Decode<'de> for DisplayControlPdu {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let payload_length = pdu_length
            ...
            .ok_or_else(
                || invalid_field_err!("Length", "Display control PDU length is too small", in: src),
            )?;
        ...
        let pdu = match kind {
            DISPLAYCONTROL_PDU_TYPE_CAPS => {
                DisplayControlPdu::Caps(DisplayControlCapabilities::decode(src)?)
            }
            ...
            _ => {
                return Err(
                    invalid_field_err!("Type", "Unknown display control PDU type", in: src),
                );
            }
        };
...
impl Encode for DisplayControlMonitorLayout {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        let monitors_count: u32 = self.monitors.len().try_into().map_err(
            |_| invalid_field_err!("NumMonitors", "Number of monitors is too big", in: dst),
        )?;
