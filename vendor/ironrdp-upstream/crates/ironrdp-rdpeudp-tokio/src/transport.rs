...
pub struct UdpTlsConfig {
    ...
    /// Exact leaf DER observed on the authenticated primary TCP TLS connection.
    /// When set, the sideband handshake must present the same certificate before RDPEMT starts.
    pub expected_leaf_der: Option<Vec<u8>>,
}
...
impl core::fmt::Debug for UdpTlsConfig {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("UdpTlsConfig")
            ...
            .field(
                "certificate_validation_endpoint",
                &self.certificate_validation_endpoint,
            )
            .field(
                "expected_leaf_der",
                &self.expected_leaf_der.as_ref().map(|_| "Some(..)"),
            )
            .finish()
    }
}
...
fn driver_exit_during_handshake(
    ...
    match join_result {
        Ok(Ok(())) => {
            UdpTransportError::handshake(context, DriverError::connection_closed(context))
        }
...
/// rejection.
fn verify_expected_leaf(expected: Option<&[u8]>, presented: Option<&[u8]>) -> std::io::Result<()> {
    if let Some(expected) = expected {
        if presented != Some(expected) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::PermissionDenied,
                "RDP-UDP TLS leaf differs from primary connection",
            ));
        }
    }
    Ok(())
...
pub async fn connect_udp(config: UdpTransportConfig) -> Result<UdpTransport, UdpTransportError> {
    ...
    let conn =
        RdpeudpConnection::connect(connection_config, Clock::new().now()).map_err(|error| {
            UdpTransportError::handshake(
                "connect UDP",
                DriverError::rdpeudp("build RDP-UDP connection", error),
            )
        })?;
    ...
    verify_expected_leaf(
        config.tls.expected_leaf_der.as_deref(),
        tls_stream
            .get_ref()
            .1
            .peer_certificates()
    ...
    W: tokio::io::AsyncWrite + Unpin,
{
    ...
    match tunnel.poll_event() {
        ...
        Some(ironrdp_rdpemt::TunnelEvent::Failed { hr_response }) => Err(
            UdpTransportError::tunnel_rejected("establish tunnel split", hr_response),
        ),
...
async fn accept_udp_inner(
    ...
    let syn_datagram: V1Datagram = ironrdp_core::decode(&recv_buf[..n]).map_err(|_| {
        UdpTransportError::handshake(
            ...
            DriverError::rdpeudp(
                "decode SYN datagram",
                ironrdp_rdpeudp::RdpeudpError::invalid_packet(
                    "decode SYN datagram",
                    "SYN datagram did not decode",
                ),
            ),
        )
    })?;
    ...
    let conn = RdpeudpConnection::accept(connection_config, &syn_datagram, Clock::new().now())
        .map_err(|error| {
            UdpTransportError::handshake(
                "accept UDP",
                DriverError::rdpeudp("accept RDP-UDP connection", error),
            )
        })?;
    ...
    // tick; check once more for a driver that died right as it signaled.
    if let Some(driver) = driver_handle.take_if_finished() {
        return Err(driver_exit_during_handshake(
            "accept udp inner",
            driver.await,
        ));
    }
    ...
    let mut tunnel =
        establish_tunnel_server_split(&mut tls_read, &mut tls_write, config.tunnel_config).await?;
    ...
    W: tokio::io::AsyncWrite + Unpin,
{
    ...
    match tunnel.poll_event() {
        Some(ironrdp_rdpemt::TunnelEvent::Established) => Ok(tunnel),
        Some(ironrdp_rdpemt::TunnelEvent::Failed { hr_response }) => Err(
            UdpTransportError::tunnel_rejected("establish tunnel server split", hr_response),
        ),
        _ => Err(UdpTransportError::rdpemt(
    ...
    W: tokio::io::AsyncWrite + Unpin,
{
    while let Some(data) = outgoing_rx.recv().await {
        ...
        let encoded = ironrdp_core::encode_vec(&pdu).map_err(|error| {
...
mod tests {
    ...
    fn driver_exit_during_handshake_reports_the_real_driver_error() {
        ...
        assert!(
            matches!(
                error.kind(),
                crate::error::UdpTransportErrorKind::Handshake(_)
            ),
    ...
    async fn driver_exit_during_handshake_reports_a_panic_distinctly() {
        ...
        assert!(
            join_result.is_err(),
            "the spawned task was supposed to panic"
        );
        ...
        assert!(
            matches!(
                error.kind(),
                crate::error::UdpTransportErrorKind::DriverPanic
            ),
...
#[cfg(test)]
mod norishell_leaf_tests {
    use super::verify_expected_leaf;

    #[test]
    fn udp_leaf_must_match_authenticated_tcp_leaf() {
        assert!(verify_expected_leaf(Some(&[1, 2, 3]), Some(&[1, 2, 3])).is_ok());
        assert!(verify_expected_leaf(Some(&[1, 2, 3]), Some(&[1, 2, 4])).is_err());
        assert!(verify_expected_leaf(Some(&[1, 2, 3]), None).is_err());
        assert!(verify_expected_leaf(None, Some(&[9])).is_ok());
    }
}
