...
mod tests {
    ...
    fn poll_write_blocks_once_write_buf_is_full() {
        let shared = Arc::new(Mutex::new(SharedIo::new()));
        {
            let mut s = shared.lock().expect("lock");
            s.write_buf
                .extend_from_slice(&vec![0u8; WRITE_BUF_HIGH_WATER]);
        }
        ...
        assert!(
            result.is_pending(),
            "poll_write should block at the high-water mark"
        );
        ...
        assert!(
            !test_waker.was_woken(),
            "not woken yet, nothing has drained write_buf"
        );
    }
    ...
    fn close_wakes_every_registered_waker() {
        ...
        assert!(
            read_drained_waker.was_woken(),
            "read_drained_waker not woken"
        );
