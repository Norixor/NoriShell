...
impl Driver {
    ...
    fn queue_write(&mut self, data: Vec<u8>) -> Result<bool, DriverError> {
        match self.conn.send(data) {
            Ok(()) => Ok(true),
            Err(SendError { error, data })
                if matches!(error.kind(), RdpeudpErrorKind::SendBufferFull) =>
            {
...
mod tests {
    ...
    fn established_client_connection() -> RdpeudpConnection {
        ...
        let mut server =
            RdpeudpConnection::accept(test_connection_config(), &syn_dg, t).expect("accept");
        ...
        assert!(
            ...
            "test setup: client should be established"
        );
        client
    }
    ...
    async fn driver_sends_initial_syn() {
        ...
        let conn = RdpeudpConnection::connect(test_connection_config(), Clock::new().now())
            .expect("connect");
        ...
        let recv_result =
            tokio::time::timeout(Duration::from_secs(2), server_sock.recv(&mut buf)).await;
    ...
    fn droppable_errors_are_the_ones_the_peer_controls() {
        assert!(is_droppable(&RdpeudpError::invalid_packet(
            "test",
            "malformed"
        )));
    ...
    fn pads_a_syn_to_the_smaller_advertised_mtu() {
        ...
        assert_eq!(
            padded.len(),
            1150,
            "should pad to the smaller of the two MTUs"
        );
        assert!(
    ...
    async fn queue_write_holds_data_instead_of_erroring_when_the_send_buffer_is_full() {
        ...
        assert!(
            !queued,
            "the write past the bound should be held, not queued"
        );
        assert_eq!(
