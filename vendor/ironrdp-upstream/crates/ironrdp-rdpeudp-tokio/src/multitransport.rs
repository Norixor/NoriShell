...
impl MultitransportBootstrap {
    ...
    pub fn from_pdu(pdu_bytes: &[u8]) -> Result<Self, UdpTransportError> {
        let request: MultitransportRequestPdu =
            ironrdp_core::decode(pdu_bytes).map_err(|error| {
                UdpTransportError::rdpemt(
                    "decode multitransport request",
                    RdpemtError::decode(error),
                )
            })?;
    ...
    pub fn response_pdu(&self) -> Option<Vec<u8>> {
        self.response.as_ref().map(|r| {
            ironrdp_core::encode_vec(r).expect("MultitransportResponsePdu encoding cannot fail")
        })
    }
...
mod tests {
    ...
    fn new_from_request() {
        let bootstrap =
            MultitransportBootstrap::new(request(42, [0xAB; 16], RequestedProtocol::UdpFecR));
    ...
    fn from_pdu_roundtrip() {
        let encoded =
            ironrdp_core::encode_vec(&request(99, [0xCC; 16], RequestedProtocol::UdpFecR))
                .expect("encode");
    ...
    async fn connect_rejects_unsupported_protocol_without_attempting_transport() {
        let mut bootstrap =
            MultitransportBootstrap::new(request(7, [0x11; 16], RequestedProtocol::UdpFecL));
        ...
        let response = bootstrap
            .response_pdu()
            .expect("response set after connect");
        let decoded: MultitransportResponsePdu =
            ironrdp_core::decode(&response).expect("decode response");
