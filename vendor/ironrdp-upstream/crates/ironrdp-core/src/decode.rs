...
/// detected. `Some(0)` therefore means byte zero of the input stream. Producers
/// with no stream cursor (a `try_from` on a primitive, a constructor, a
...
pub enum DecodeErrorKind {
    ...
    NotEnoughBytes {
        ...
        ///
        /// `None` when the error has no position in the input stream at all:
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NotEnoughBytes {
                ...
                f,
                "not enough bytes provided to decode{at}: received {received} bytes, expected {expected} bytes",
                at = At(*offset)
            ),
            Self::InvalidField {
                field,
...
impl UnsupportedValueErr for DecodeError {
    ...
    fn unsupported_value(
        ...
        Self::new(
            context,
            DecodeErrorKind::UnsupportedValue {
                name,
                value,
                offset,
            },
