...
/// Structured encode errors carry the fields needed to describe the failure,
...
/// detected. `Some(0)` therefore means byte zero of the output stream. Producers
/// with no stream cursor (a constructor, an integer conversion, a cache-state
/// failure) pass `None`, which is not a placeholder for a position that was
/// merely unavailable.
///
/// [`EncodeErrorKind::Other`] is reserved for errors that do not fit one of the
...
pub enum EncodeErrorKind {
    ...
    NotEnoughBytes {
        ...
        expected: usize,
        /// Byte offset in the output stream where the shortage was detected.
        ///
        /// `None` when the error has no position in the output stream at all:
        /// a constructor validating its arguments, an integer conversion, a
        /// cache-state failure. Not a placeholder for a position that was
        /// merely unavailable.
        offset: Option<usize>,
    },
    ...
    InvalidField {
        ...
        reason: &'static str,
        /// Byte offset in the output stream where the invalid field was being written.
        ///
        /// `None` when the error has no position in the output stream at all.
        offset: Option<usize>,
    },
    ...
    UnexpectedMessageType {
        ...
        got: u8,
        /// Byte offset in the output stream where the unexpected type was written.
        ///
        /// `None` when the error has no position in the output stream at all.
        offset: Option<usize>,
    },
    ...
    UnsupportedVersion {
        ...
        got: u8,
        /// Byte offset in the output stream where the unsupported version was written.
        ///
        /// `None` when the error has no position in the output stream at all.
        offset: Option<usize>,
    },
    ...
    UnsupportedValue {
        ...
        value: String,
        /// Byte offset in the output stream where the unsupported value was written.
        ///
        /// `None` when the error has no position in the output stream at all.
        offset: Option<usize>,
    },
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NotEnoughBytes {
                ...
                f,
                "not enough bytes provided to encode{at}: received {received} bytes, expected {expected} bytes",
                at = At(*offset)
            ),
            Self::InvalidField {
                field,
                ...
                write!(f, "invalid `{field}`{at}: {reason}", at = At(*offset))
            }
            Self::UnexpectedMessageType { got, offset } => {
                write!(f, "invalid message type ({got}){at}", at = At(*offset))
            }
            Self::UnsupportedVersion { got, offset } => {
                write!(f, "unsupported version ({got}){at}", at = At(*offset))
            }
            #[cfg(feature = "alloc")]
            Self::UnsupportedValue {
                ...
                write!(f, "unsupported {name} ({value}){at}", at = At(*offset))
            }
            #[cfg(not(feature = "alloc"))]
            Self::UnsupportedValue { name, offset } => {
                write!(f, "unsupported {name}{at}", at = At(*offset))
            }
...
impl NotEnoughBytesErr for EncodeError {
    ...
    fn not_enough_bytes(
        ...
        Self::new(
            context,
            EncodeErrorKind::NotEnoughBytes {
                received,
                expected,
                offset,
            },
...
impl UnexpectedMessageTypeErr for EncodeError {
    #[track_caller]
    fn unexpected_message_type(context: &'static str, got: u8, offset: Option<usize>) -> Self {
        Self::new(
            context,
            EncodeErrorKind::UnexpectedMessageType { got, offset },
        )
    }
}
...
impl UnsupportedVersionErr for EncodeError {
    #[track_caller]
    fn unsupported_version(context: &'static str, got: u8, offset: Option<usize>) -> Self {
        Self::new(context, EncodeErrorKind::UnsupportedVersion { got, offset })
    }
}
...
impl UnsupportedValueErr for EncodeError {
    ...
    fn unsupported_value(
        ...
        Self::new(
            context,
            EncodeErrorKind::UnsupportedValue {
                name,
                value,
                offset,
            },
    ...
    #[cfg(not(feature = "alloc"))]
    #[track_caller]
    fn unsupported_value(context: &'static str, name: &'static str, offset: Option<usize>) -> Self {
        Self::new(context, EncodeErrorKind::UnsupportedValue { name, offset })
    }
}
...
mod legacy {
    ...
    use crate::{WriteCursor, ensure_size};
