...
/// Renders an optional stream position for `Display`.
///
/// Emits ` at offset N` when the error has a position and nothing at all when it
/// does not, so a constructor or cast failure never claims to have happened at
/// byte zero.
pub(crate) struct At(pub(crate) Option<usize>);
...
pub trait NotEnoughBytesErr {
    ...
    ///   `None` when the producer has no stream cursor.
    ///
    /// # Returns
    ///
