...
use ironrdp_core::{
    ...
    UnexpectedMessageTypeErr as _, WriteCursor,
};
...
impl Encode for TunnelSubHeader {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        let sub_header_length = u8::try_from(total).map_err(|_| {
            ironrdp_core::invalid_field_err!(
                Self::NAME,
                "SubHeaderLength",
                "sub-header exceeds 255 bytes"
            )
        })?;
...
impl Decode<'_> for TunnelSubHeader {
    fn decode(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ...
        let sub_header_type = SubHeaderType::from_u8(type_raw).ok_or_else(|| {
            ironrdp_core::DecodeError::unexpected_message_type(
                Self::NAME,
                type_raw,
                Some(src.pos()),
            )
