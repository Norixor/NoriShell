...
use alloc::vec::Vec;
use ironrdp_core::{
    Decode, DecodeResult, Encode, EncodeResult, ReadCursor, UnexpectedMessageTypeErr as _,
    WriteCursor,
};
...
impl TunnelData {
    ...
    fn header_length_untruncated(&self) -> usize {
        TunnelHeader::MIN_SIZE
            + self
                .sub_headers
                .iter()
                .map(TunnelSubHeader::wire_size)
                .sum::<usize>()
    }
    ...
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        let header_length = self.header_length().ok_or_else(|| {
            ironrdp_core::invalid_field_err!(
                Self::NAME,
                "HeaderLength",
                "sub-headers exceed one byte of length"
            )
        })?;
