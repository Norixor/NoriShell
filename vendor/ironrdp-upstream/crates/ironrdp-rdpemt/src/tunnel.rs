...
use crate::pdu::{
    TunnelCreateRequest, TunnelCreateResponse, TunnelData, TunnelPdu, TunnelSubHeader,
};
...
impl RdpemtTunnel {
    ...
    pub fn handle_pdu(&mut self, data: &[u8]) -> Result<(), RdpemtError> {
        ...
        match (&self.state, &self.side, pdu) {
            ...
            (TunnelState::Created, Side::Server, TunnelPdu::CreateRequest(req)) => {
                ...
                let cookie_matches: bool = req
                    .security_cookie
                    .ct_eq(&self.config.security_cookie)
                    .into();
                if req.request_id == self.config.request_id && cookie_matches {
                    ...
                    self.events
                        .push_back(TunnelEvent::Failed { hr_response: hr });
                }
            }
    ...
    fn enqueue_create_request(&mut self) {
        ...
        let encoded = ironrdp_core::encode_vec(&pdu)
            .expect("CreateRequest encoding is infallible for valid inputs");
        self.outgoing.push_back(encoded);
    ...
    fn enqueue_create_response(&mut self, hr_response: u32) {
        ...
        let encoded = ironrdp_core::encode_vec(&pdu)
            .expect("CreateResponse encoding is infallible for valid inputs");
        self.outgoing.push_back(encoded);
    }
}
