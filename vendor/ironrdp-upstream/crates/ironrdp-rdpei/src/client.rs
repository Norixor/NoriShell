...
use crate::pdu::{
    CsReadyFlags, CsReadyPdu, DismissHoveringTouchContactPdu, PenEventPdu, RdpInputProtocolVersion,
    RdpeiPdu, ScReadyFeatures, TouchEventPdu,
};
    ...
    fn process(&mut self, _channel_id: u32, payload: &[u8]) -> PduResult<Vec<DvcMessage>> {
        ...
        match pdu {
            RdpeiPdu::ScReady(sc) => {
                ...
                self.negotiated_version =
                    Some(core::cmp::min(sc.protocol_version, cs.protocol_version));
                self.ready = true;
                debug!(
            ...
            | RdpeiPdu::Touch(_)
            | RdpeiPdu::Pen(_)
            | RdpeiPdu::DismissHoveringTouchContact(_) => Err(pdu_other_err!(
                "RdpeiClient::process",
