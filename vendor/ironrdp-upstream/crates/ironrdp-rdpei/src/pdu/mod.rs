...
use ironrdp_core::{
    Decode, DecodeResult, Encode, EncodeResult, ReadCursor, WriteCursor, cast_int, cast_length,
    ensure_fixed_part_size, ensure_size, invalid_field_err,
};
use ironrdp_dvc::DvcEncode;

pub use self::varint::{
    EightByteUnsigned, FourByteSigned, FourByteUnsigned, TwoByteSigned, TwoByteUnsigned,
};
...
impl TouchContact {
    ...
    pub fn with_contact_rect(mut self, left: i16, top: i16, right: i16, bottom: i16) -> Self {
        self.fields_present
            .insert(TouchContactDataFlags::CONTACTRECT_PRESENT);
    ...
    pub fn with_orientation(mut self, orientation: u32) -> Self {
        self.fields_present
            .insert(TouchContactDataFlags::ORIENTATION_PRESENT);
    ...
    pub fn with_pressure(mut self, pressure: u32) -> Self {
        self.fields_present
            .insert(TouchContactDataFlags::PRESSURE_PRESENT);
...
impl Encode for TouchContact {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        if self
            .fields_present
            .contains(TouchContactDataFlags::CONTACTRECT_PRESENT)
        {
            let left = self
                .fields
                .contact_rect_left
        ...
        if self
            .fields_present
            .contains(TouchContactDataFlags::ORIENTATION_PRESENT)
        {
        ...
        if self
            .fields_present
            .contains(TouchContactDataFlags::PRESSURE_PRESENT)
        {
            let pressure = self
                .fields
                .pressure
...
impl<'de> Decode<'de> for TouchContact {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let fields_present =
            TouchContactDataFlags::from_bits_truncate(TwoByteUnsigned::decode(src)?.get());
        let x = FourByteSigned::decode(src)?.get();
        let y = FourByteSigned::decode(src)?.get();
        let contact_flags =
            TouchContactFlags::from_bits_truncate(FourByteUnsigned::decode(src)?.get());
...
impl PenContact {
    ...
    pub fn with_pen_flags(mut self, pen_flags: PenFlags) -> Self {
        self.fields_present
            .insert(PenContactDataFlags::PENFLAGS_PRESENT);
    ...
    pub fn with_pressure(mut self, pressure: u32) -> Self {
        self.fields_present
            .insert(PenContactDataFlags::PRESSURE_PRESENT);
        self.fields.pressure = Some(pressure);
        self
    }
    ...
    pub fn with_rotation(mut self, rotation: u16) -> Self {
        self.fields_present
            .insert(PenContactDataFlags::ROTATION_PRESENT);
...
impl Encode for PenContact {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        if self
            .fields_present
            .contains(PenContactDataFlags::PENFLAGS_PRESENT)
        {
            let pen_flags = self
                .fields
                .pen_flags
        ...
        if self
            .fields_present
            .contains(PenContactDataFlags::PRESSURE_PRESENT)
        {
            let pressure = self
                .fields
                .pressure
        ...
        if self
            .fields_present
            .contains(PenContactDataFlags::ROTATION_PRESENT)
        {
            let rotation = self
                .fields
                .rotation
        ...
        if self
            .fields_present
            .contains(PenContactDataFlags::TILTX_PRESENT)
        {
            let tilt_x = self
                .fields
                .tilt_x
        ...
        if self
            .fields_present
            .contains(PenContactDataFlags::TILTY_PRESENT)
        {
            let tilt_y = self
                .fields
                .tilt_y
...
impl<'de> Decode<'de> for PenContact {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let fields_present =
            PenContactDataFlags::from_bits_truncate(TwoByteUnsigned::decode(src)?.get());
        let x = FourByteSigned::decode(src)?.get();
        let y = FourByteSigned::decode(src)?.get();
        let contact_flags =
            PenContactFlags::from_bits_truncate(FourByteUnsigned::decode(src)?.get());

        let mut fields = PenContactFields::default();
        if fields_present.contains(PenContactDataFlags::PENFLAGS_PRESENT) {
            fields.pen_flags = Some(PenFlags::from_bits_truncate(
                FourByteUnsigned::decode(src)?.get(),
            ));
        }
...
mod tests {
    ...
    fn sc_ready_round_trip() {
        ...
        assert_eq!(
            ...
            RdpeiPdu::ScReady(pdu)
        );
    }

    #[test]
    fn cs_ready_round_trip() {
        let pdu = CsReadyPdu::new(
    ...
    fn suspend_resume_dismiss_round_trip() {
        ...
        assert_eq!(
            decode::<RdpeiPdu>(&suspend).unwrap(),
