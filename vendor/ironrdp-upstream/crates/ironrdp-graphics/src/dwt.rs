...
fn horizontal_band<const SUBBAND_WIDTH: usize>(low: &[i16], high: &[i16], dst: &mut [i16]) {
    ...
    while n < sw {
        vst(
            &mut ev,
            n,
            vld(low, n) - ceil_avg(vld(&high_pad, n), vld(high, n)),
        );
        n += 8;
    }
    ...
    od[sw - 1] =
        i32_to_i16_possible_truncation((i32::from(high[sw - 1]) << 1) + i32::from(ev[sw - 1]));
...
fn inverse_vertical<const SUBBAND_WIDTH: usize>(buffer: &mut [i16], temp_buffer: &[i16]) {
    ...
    while cb < tw {
        ...
        vst(
            buffer,
            cb,
            vld(temp_buffer, cb) - vld(temp_buffer, cb + sw * tw),
        );
