...
impl core::fmt::Display for SrlError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::ZeroRunTooLong => {
                write!(f, "srl zero run exceeds the component coefficient count")
            }
        }
    }
}
...
pub struct SrlDecoder<'a> {
    ...
    zero_run_total: usize,
    nonzero_pending: bool,
}

impl<'a> SrlDecoder<'a> {
    /// Create a decoder for the complete length-delimited SRL field.
    /// A final zero byte may contain magnitude bits, so it cannot be stripped.
    ...
    pub fn decode(&mut self, num_values: usize, num_bits: u8) -> Result<Vec<i16>, SrlError> {
        ...
        while output.len() < num_values {
            ...
            if self.nonzero_pending {
                ...
                self.zero_run_total = 0;
                continue;
            }

            let (chunk, has_nonzero) = self.decode_zero_chunk()?;
            self.zero_run_total = self
                .zero_run_total
                .checked_add(chunk)
                .filter(|total| *total <= MAX_ZERO_RUN)
                .ok_or(SrlError::ZeroRunTooLong)?;
            self.zero_run_remaining = chunk;
            self.nonzero_pending = has_nonzero;
        }

        Ok(output)
    }

    fn decode_zero_chunk(&mut self) -> Result<(usize, bool), SrlError> {
        let k = self.kp / 8;
...
impl SrlEncoder {
    ...
    /// Finish the stream, adding the conventional trailing zero byte.
    ...
    fn encode_zero_run(&mut self, mut zeros: usize) -> Result<(), SrlError> {
        ...
        self.writer.write_bits(
            u32::try_from(zeros).map_err(|_| SrlError::ZeroRunTooLong)?,
            k,
        );
        self.kp = self.kp.saturating_sub(6);
...
mod tests {
    ...
    #[test]
    fn decodes_final_zero_byte_as_payload() {
        assert_eq!(decode_srl(&[0x80, 0x00, 0x00], 1, 4), Ok(vec![15]));
    }

    #[test]
    fn decodes_complete_zero_chunk_without_lookahead() {
        assert_eq!(decode_srl(&[0x00], 2, 4), Ok(vec![0, 0]));

        let mut decoder = SrlDecoder::new(&[0x00]).unwrap();
        assert_eq!(decoder.decode(1, 4), Ok(vec![0]));
        assert_eq!(decoder.decode(1, 4), Ok(vec![0]));
    }

    #[test]
    fn decodes_stream_without_optional_terminator() {
    ...
    fn rejects_out_of_range_magnitude() {
        assert_eq!(
            encode_srl(&[8], 3),
            Err(SrlError::MagnitudeOutOfRange {
                magnitude: 8,
                max: 7
            })
        );
    ...
    fn rejects_zero_run_longer_than_component() {
        assert_eq!(
            encode_srl(&vec![0; MAX_ZERO_RUN + 1], 1),
    ...
    fn round_trips_mixed_values() {
        ...
        assert_eq!(
            decode_srl(&encoded, original.len(), 4),
            Ok(original.to_vec())
        );
    }
