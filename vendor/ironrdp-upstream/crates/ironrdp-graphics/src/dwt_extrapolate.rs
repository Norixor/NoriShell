...
#[expect(
    ...
    reason = "hl/lh/hh/ll are standard DWT subband names"
)]
...
fn dwt_row_scattered(
    ...
    if n_l <= n_h + 1 {
        ...
        out[low_off + n_h + 1] =
            t(2 * i32::from(input[n_l + n_h - 1]) - i32::from(input[n_l + n_h - 2]));
    }
}
...
mod tests {
    ...
    fn round_trip_identity() {
        ...
        assert!(
            max_err <= 2,
            "max round-trip error {max_err} exceeds tolerance"
        );
    }
