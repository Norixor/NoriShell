...
pub fn rustls_client_config(
    ...
    let mut config = rustls::ClientConfig::builder()
        ...
        .with_no_client_auth();
    // Never write RDP TLS traffic secrets through SSLKEYLOGFILE.
...
mod norishell_tests {
    ...
    #[derive(Debug)]
    struct RejectCertificate(rustls::CertificateError);

    impl ServerCertVerifier for RejectCertificate {
        fn verify_server_cert(
            &self,
            _: &rustls::pki_types::CertificateDer<'_>,
            _: &[rustls::pki_types::CertificateDer<'_>],
    ...
    #[test]
    fn callback_cannot_override_expired_exact_leaf() {
        use std::sync::atomic::{AtomicUsize, Ordering};
        let calls = Arc::new(AtomicUsize::new(0));
        for (rejection, should_accept) in [
            (rustls::CertificateError::Expired, false),
            (rustls::CertificateError::UnknownIssuer, true),
        ] {
            let callback_calls = Arc::clone(&calls);
            let verifier = CallbackVerifier {
                verifier: Arc::new(RejectCertificate(rejection)),
                endpoint: "example.invalid".to_owned(),
                callback: Arc::new(move |_, _, _| {
    ...
    #[test]
    fn tls_client_config_never_enables_key_logging() {
