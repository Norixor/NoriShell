...
pub use v1_ack::{
    CorrelationIdPayload, V1AckOfAcksHeader, V1AckVectorElement, V1AckVectorHeader,
    VectorElementState,
};
...
impl Encode for V1Datagram {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        if let Some(syn_data_ex) = &self.syn_data_ex {
            ...
            let decoder_expects_cookie_hash =
                is_client_syn && syn_data_ex.udp_ver == UdpVersion::V3;
...
impl Decode<'_> for V1Datagram {
    fn decode(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ...
        // note on `V1Datagram`.
        let ack_vector =
            if header.flags.contains(V1Flags::ACK) && !header.flags.contains(V1Flags::SYN) {
        ...
        let syn_data_ex = if header.flags.contains(V1Flags::SYNEX) {
            ...
            let is_client_syn =
                header.flags.contains(V1Flags::SYN) && !header.flags.contains(V1Flags::ACK);
