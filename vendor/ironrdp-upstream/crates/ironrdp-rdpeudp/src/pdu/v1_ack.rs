...
impl V1AckVectorHeader {
    ...
    fn padding_len(element_count: usize) -> usize {
        (Self::FIXED_PART_SIZE + element_count).next_multiple_of(4)
...
impl Encode for V1AckVectorHeader {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        let count: u16 = ironrdp_core::cast_length!(
            "RDPUDP_ACK_VECTOR_HEADER",
            "uAckVectorSize",
            self.elements.len()
        )?;
...
mod tests {
    ...
    fn ack_vector_element_length_is_masked_not_overflowed() {
        ...
        assert_eq!(
            elem.to_byte() >> 6,
            0,
            "length overflowed into the state bits"
        );
    }
    ...
    fn ack_vector_roundtrip() {
        for count in [0usize, 1, 2, 3, 4, 5, 9] {
            ...
            assert_eq!(
                encoded.len() % 4,
                0,
                "structure must end on a DWORD boundary"
            );
            assert_eq!(
                decode::<V1AckVectorHeader>(&encoded).expect("decode"),
                original
            );
        }
    }
    ...
    fn ack_of_acks_roundtrip() {
        ...
        assert_eq!(
            decode::<V1AckOfAcksHeader>(&encoded).expect("decode"),
            original
        );
    }
    ...
    fn correlation_id_roundtrip() {
        ...
        assert_eq!(
            decode::<CorrelationIdPayload>(&encoded).expect("decode"),
            original
        );
    }
