...
impl RdpeudpConnection {
    ...
    pub fn accept(
        ...
        let syn_data = syn_datagram.syn_data.as_ref().ok_or_else(|| {
            RdpeudpError::invalid_packet("accept", "SYN datagram missing SynData payload")
        })?;

        let syn_data_ex = syn_datagram.syn_data_ex.as_ref().ok_or_else(|| {
            RdpeudpError::invalid_packet("accept", "SYN datagram missing SynDataEx payload")
        })?;
        ...
        let offered_hash = syn_data_ex.cookie_hash.ok_or_else(|| {
            RdpeudpError::invalid_packet("accept", "version 3 SYN carries no cookieHash")
        })?;
    ...
    pub fn send(&mut self, data: Vec<u8>) -> Result<(), SendError> {
        ...
        let max_entries =
            SEND_BUFFER_WINDOW_MULTIPLE * (1usize << usize::from(self.config.log_window_size));
    ...
    pub fn max_payload(&self) -> usize {
        if let Some(params) = self
            .params
            .as_ref()
            .filter(|p| matches!(p.wire, WireFormat::V1 { .. }))
        {
            return usize::from(params.mtu)
                .saturating_sub(V1_DATA_OVERHEAD)
                .max(1);
        }

        let mtu = self.params.as_ref().map_or(RDPEUDP2_MTU, |params| {
            usize::from(params.mtu).min(RDPEUDP2_MTU)
        });

        mtu.saturating_sub(DATA_PACKET_OVERHEAD + PIGGYBACK_RESERVE)
            .max(1)
    }
    ...
    pub fn poll_transmit(&mut self, now: MonotonicInstant) -> Option<Transmit> {
        ...
        if let Some(t) = self.pending_transmits.pop_front() {
            self.timers
                .set(Timer::KeepAlive, now + self.config.keep_alive_interval);
            return Some(t);
        }
    ...
    fn wire_is_v1(&self) -> bool {
        matches!(
            self.params.as_ref().map(|p| p.wire),
            Some(WireFormat::V1 { .. })
        )
    }
    ...
    fn enqueue_syn(&mut self, now: MonotonicInstant) {
        ...
        self.timers
            .set(Timer::Retransmit, now + self.effective_rto());
        self.timers.set(Timer::Idle, now + self.config.idle_timeout);
    }
    ...
    fn handle_syn_ack(&mut self, wire: &[u8], now: MonotonicInstant) -> Result<(), RdpeudpError> {
        ...
        let syn_data = datagram.syn_data.as_ref().ok_or_else(|| {
            RdpeudpError::invalid_packet("handle SYN+ACK", "SYN+ACK missing SynData")
        })?;

        let syn_data_ex = datagram.syn_data_ex.as_ref().ok_or_else(|| {
            RdpeudpError::invalid_packet("handle SYN+ACK", "SYN+ACK missing SynDataEx")
        })?;
    ...
    fn process_ack(&mut self, ack: &AckPayload, _header: &V2Header, now: MonotonicInstant) {
        ...
        self.remote_timestamp_ref =
            seq::reconstruct_timestamp(ack.received_ts, self.remote_timestamp_ref);
    ...
    fn process_ack_vector(
        &mut self,
        ack_vector: &AckVectorPayload,
        _header: &V2Header,
        now: MonotonicInstant,
    ) {
        let send_window = match self.send_window.as_mut() {
                    ...
                    for _ in 0..u64::from(*length) {
                        if *received {
                            ...
                            if let Some(size) = send_window.mark_received(current_seq) {
                                newly_acked_bytes +=
                                    u64::try_from(size).expect("packet size fits in u64");
                            }
                        }
                        current_seq += 1;
                    }
    ...
    fn process_dummy_data(&mut self, dh: &DataHeader, now: MonotonicInstant) {
        ...
        if !self.timers.is_set(Timer::AckDelay) {
            self.timers
                .set(Timer::AckDelay, now + self.ack_delay_timeout());
    ...
    fn run_loss_detection(&mut self, now: MonotonicInstant) {
        ...
        let lost_seqs = self
            .loss_detector
            .detect(send_window, highest_acked, rto, now);
    ...
    fn try_retransmit(&mut self, now: MonotonicInstant) -> Option<Transmit> {
        ...
        let new_data_seq =
            send_window.push_retransmit(entry.channel_seq, entry.data.clone(), now)?;
    ...
    fn build_ack_payload(&self, recv_window: &RecvWindow, now: MonotonicInstant) -> AckPayload {
        ...
        AckPayload {
            ...
            send_ack_time_gap: u8::try_from(now.duration_since(received_at).as_millis())
                .unwrap_or(u8::MAX),
    ...
    fn handle_v1_datagram(
        ...
        if datagram.header.flags.contains(V1Flags::ACK) {
            self.process_v1_acknowledgement(
                &datagram.header,
                datagram.ack_vector.as_ref(),
                delayed,
                now,
            );
        }
        ...
        if let Some(vector) = ack_vector {
            ...
            let pending: BTreeMap<u64, (u64, bool, MonotonicInstant)> = send_window
                ...
                .map(|e| {
                    (
                        e.channel_seq,
                        (e.data_seq, e.transmit_count == 1, e.sent_at),
                    )
                })
                .collect();
    ...
    fn build_v1_ack_parts(&self, flags: V1Flags) -> Option<(FecHeader, V1AckVectorHeader)> {
        ...
        'runs: for (received, count) in recv_window.ack_vector().into_iter().rev() {
            ...
            while remaining > 0 {
                ...
                let chunk =
                    u8::try_from(remaining.min(u64::from(V1AckVectorElement::MAX_LENGTH) + 1))
...
mod tests {
    ...
    fn ack_builders_report_the_real_delay_gap_and_timestamp() {
        ...
        let expected_ts = seq::truncate_timestamp(RdpeudpConnection::timestamp_units(
            MonotonicInstant::from_millis(100),
        ));
...
mod v1_tests {
    ...
    fn version_2_syn_ack_selects_the_rdpeudp_framing() {
        ...
        assert_eq!(
            conn.params.as_ref().unwrap().wire,
            WireFormat::V1 { version: 2 }
        );
    ...
    fn data_goes_out_as_a_source_packet_with_an_ack() {
        ...
        assert_eq!(
            datagram.header.sn_source_ack, REMOTE_ISN,
        ...
        assert!(
            conn.poll_transmit(at(31)).is_none(),
            "single packet, no ack pending"
        );
    }
    ...
    fn windows_ack_vector_runs_are_count_minus_one() {
        ...
        assert_eq!(
            pending(&conn),
            0,
            "element 0x00 acknowledges exactly one datagram"
        );
        ...
        assert_eq!(
            pending(&conn),
            0,
            "element 0x03 acknowledges seq 4 down to seq 1"
        );
    ...
    fn an_unacknowledged_source_packet_is_retransmitted_with_a_new_coded_number() {
        ...
        assert_eq!(
            data.header.sn_coded,
            LOCAL_ISN + 2,
            "new Coded sequence number"
        );
        assert_eq!(data.payload, b"hello");
    }
    ...
    fn a_syn_ack_selecting_a_version_above_the_offer_is_rejected() {
        ...
        conn.handle_datagram(&mut wire, at(20)).expect_err(
            "the SYN offered version 2, so version 3 is not supported by both endpoints",
        );
    }
    ...
    fn an_ack_of_acks_ahead_of_the_receive_window_is_ignored() {
        ...
        assert_eq!(
            conn.poll_event(),
            Some(Event::DataReceived(b"hello ".to_vec()))
        );
