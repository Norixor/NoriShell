...
pub(crate) struct LostPacketInfo {
    ...
    #[cfg_attr(
        not(test),
        expect(dead_code, reason = "diagnostic field used in tests")
    )]
    pub size: usize,
}
...
impl SendWindow {
    ...
    pub(crate) fn pending_entries(&self) -> impl Iterator<Item = &SendEntry> {
        self.entries
            .iter()
            .filter(|e| e.state == SendEntryState::Pending)
    }
    ...
    pub(crate) fn lowest_unacknowledged(&self) -> u64 {
        self.entries
            .front()
            .map_or(self.next_data_seq, |entry| entry.data_seq)
    }
...
mod tests {
    ...
    fn retransmit_then_ack() {
        ...
        // Retransmit it
        let new_dsn = w
            .push_retransmit(100, info.data, later(t, 500))
            .expect("has cap");
