...
impl RecvWindow {
    ...
    pub(crate) fn new(
        initial_data_seq: u64,
        initial_channel_seq: u64,
        log_window_size: u8,
    ) -> Self {
    ...
    pub(crate) fn advance_base(&mut self, new_base: u64) {
        ...
        let drain_count = usize::try_from(
            advance_by.min(u64::try_from(self.entries.len()).expect("entries len fits in u64")),
        )
    ...
    pub(crate) fn has_gaps(&self) -> bool {
        self.entries
            .iter()
            .any(|e| e.state == RecvEntryState::Pending)
    }
...
mod tests {
    ...
    fn the_packet_that_drains_the_buffer_is_never_refused() {
        ...
        assert_eq!(
            w.reorder_buf_len(),
            window,
            "the buffer should be at its cap"
        );
    ...
    fn reorder_buffer_is_bounded_by_the_window() {
        ...
        assert!(
            w.reorder_buf_len() <= window,
            "the reorder buffer grew past the window"
        );
    }
