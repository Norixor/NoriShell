    ...
    pub(crate) fn update(&mut self, rtt: Duration) {
        match self.srtt {
            ...
            Some(srtt) => {
                ...
                self.rttvar = self.rttvar.mul_f64(1.0 - 1.0 / f64::from(RTTVAR_BETA))
                    + abs_diff / RTTVAR_BETA;

                self.srtt =
                    Some(srtt.mul_f64(1.0 - 1.0 / f64::from(SRTT_ALPHA)) + rtt / SRTT_ALPHA);
            }
        }
...
mod tests {
    ...
    fn convergence_with_stable_rtt() {
        ...
        assert!(
            rttvar_us < 5_000,
            "expected small RTTVAR, got {rttvar_us}μs"
        );
    }
    ...
    fn jittery_samples() {
        ...
        assert!(
            rttvar_ms > 5,
            "expected non-trivial RTTVAR, got {rttvar_ms}ms"
        );
    }
