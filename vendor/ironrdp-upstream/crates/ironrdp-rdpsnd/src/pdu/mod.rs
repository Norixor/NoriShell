...
impl AudioFormat {
    ...
    pub fn matches_for_negotiation(&self, other: &Self) -> bool {
        ...
        if self.format == WaveFormat::PCM {
            ...
            self.n_avg_bytes_per_sec == other.n_avg_bytes_per_sec
                && self.n_block_align == other.n_block_align
        }
    }
}
...
impl WavePdu {
    ...
    fn decode(src: &mut ReadCursor<'_>, body_size: u16) -> DecodeResult<Self> {
        ...
        let audio_length = body_size.checked_sub(8).ok_or_else(
            || invalid_field_err!("BodySize", "WaveInfo body_size is too small", in: src),
        )?;
    ...
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        if self.audio_length > Self::MAX_AUDIO_LENGTH {
            return Err(other_err!(
                "WavePdu",
                "audio_length leaves BodySize outside u16 range"
            ));
        }
