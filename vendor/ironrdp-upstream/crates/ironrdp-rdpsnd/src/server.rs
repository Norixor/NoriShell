...
pub trait RdpsndServerHandler: Send + core::fmt::Debug {
    ...
    fn choose_format<'a>(&mut self, common: &'a [NegotiatedFormat])
        ...
        let msg = if version >= pdu::Version::V8 {
            ...
            if data.len() < usize::from(pdu::WavePdu::MIN_AUDIO_LENGTH) {
                return Err(pdu_other_err!(
                    "wave data shorter than WaveInfo Data prefix"
                ));
            }
            ...
            let audio_length =
                u16::try_from(data.len()).map_err(|_| pdu_other_err!("wave data too large"))?;
