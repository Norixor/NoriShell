...
use crate::pdu::{
    Avc420BitmapStream, CacheImportReplyPdu, CacheToSurfacePdu, CapabilitiesAdvertisePdu,
    CapabilitiesV8Flags, CapabilitiesV81Flags, CapabilitiesV107Flags, CapabilitySet, Codec1Type,
    DeleteEncodingContextPdu, EvictCacheEntryPdu, FrameAcknowledgePdu, GfxPdu,
    MapSurfaceToScaledOutputPdu, MapSurfaceToScaledWindowPdu, MapSurfaceToWindowPdu, PixelFormat,
    QueueDepth, RawCapabilitySet, SolidFillPdu, SurfaceToCachePdu, SurfaceToSurfacePdu,
    WireToSurface2Pdu,
};
...
pub trait GraphicsPipelineHandler: Send {
    ...
    /// Called after a WireToSurface2 payload has decoded successfully. Its output
    /// bitmap updates use Uncompressed as the pixel format, not the wire codec.
...
impl GraphicsPipelineClient {
    ...
    fn handle_pdu(&mut self, pdu: GfxPdu) -> PduResult<Vec<DvcMessage>> {
        match pdu {
            ...
            GfxPdu::SurfaceToCache(pdu) => {
                ...
                self.compositor.surface_to_cache(
                    pdu.surface_id,
                    pdu.cache_slot,
                    &pdu.source_rectangle,
                );
            ...
            GfxPdu::CacheToSurface(pdu) => {
                ...
                self.compositor.cache_to_surface(
                    pdu.cache_slot,
                    pdu.surface_id,
                    &pdu.destination_points,
                );
    ...
    fn handle_map_surface_to_scaled_output(&mut self, pdu: &MapSurfaceToScaledOutputPdu) {
        if let Some(surface) = self.surfaces.get_mut(&pdu.surface_id) {
            ...
            self.handler.on_surface_mapped(
                pdu.surface_id,
                pdu.output_origin_x,
                pdu.output_origin_y,
            );
    ...
    fn handle_wire_to_surface2(&mut self, pdu: WireToSurface2Pdu) -> PduResult<()> {
        ...
        let tiles = self
            ...
            .map_err(|error| {
                #[cfg(debug_assertions)]
                {
                    ...
                    {
                        let _ = writeln!(file, "{} progressive_error {error}", std::process::id());
                    }
                }
        ...
        let decoded_tiles = !tiles.is_empty();
        for tile in tiles {
            ...
            for destination_rectangle in tile.update_rectangles {
                ...
                for row in 0..usize::from(height) {
                    let source_start =
                        (source_y + row) * source_stride + source_x * TILE_BYTES_PER_PIXEL;
        ...
        if decoded_tiles {
            self.handler.on_progressive_decoded();
        }
        Ok(())
    }
    ...
    fn decode_avc420(
        ...
        self.compositor
            .apply_bitmap(surface_id, dest_rect, &update.data);
        self.handler.on_bitmap_updated(&update);
        Ok(())
    }
...
fn convert_rgb24_to_rgba(src: &[u8]) -> Vec<u8> {
    debug_assert!(
        ...
        "RGB24 input length not aligned to 3 bytes"
    );
...
fn convert_bgra_to_rgba(src: &[u8]) -> Vec<u8> {
    debug_assert!(
        ...
        "BGRA input length not aligned to 4 bytes"
    );
...
mod tests {
    ...
    fn map_surface_to_scaled_output_dispatches_to_compositor_and_handler() {
        ...
        client
            ...
            .unwrap();
        client
            .handle_pdu(GfxPdu::MapSurfaceToScaledOutput(
        ...
        assert_eq!(
            *mappings.lock().expect("mappings lock"),
            vec![(1, 3, 4, 4, 4)]
        );
        assert_eq!(
        ...
        assert_eq!(
            (
                client.surfaces[&1].output_origin_x,
                client.surfaces[&1].output_origin_y
            ),
            (3, 4)
        );
    ...
    fn clearcodec_and_planar_feed_the_compositor() {
        ...
        for (codec_id, bitmap_data) in [
            ...
            let _ = client.handle_pdu(GfxPdu::MapSurfaceToOutput(
                crate::pdu::MapSurfaceToOutputPdu {
                    surface_id: 1,
            ...
            assert_eq!(
                updates.len(),
                1,
                "{codec_id:?}: the decode must reach the compositor"
            );
    ...
    fn wire_to_surface2_renders_and_crops_progressive_edge_tile() {
        ...
        client
            ...
            .unwrap();
        client
            .handle_pdu(GfxPdu::MapSurfaceToOutput(
                crate::pdu::MapSurfaceToOutputPdu {
                    surface_id: 1,
                    output_origin_x: 0,
                    output_origin_y: 0,
    ...
    fn progressive_context_is_deleted_with_surface() {
        assert_progressive_context_is_deleted(|client| {
            client
                .handle_pdu(GfxPdu::DeleteSurface(crate::pdu::DeleteSurfacePdu {
                    surface_id: 1,
                }))
                .unwrap();
            client
