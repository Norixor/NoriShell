    ...
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        let (cmdid, payload_length) = match self {
            ...
            GfxPdu::MapSurfaceToScaledOutput(pdu) => {
                (RDPGFX_CMDID_MAPSURFACETOSCALEDOUTPUT, pdu.size())
            }
            GfxPdu::MapSurfaceToScaledWindow(pdu) => {
                (RDPGFX_CMDID_MAPSURFACETOSCALEDWINDOW, pdu.size())
            }
        };
        ...
        #[expect(
            ...
            reason = "guaranteed by GfxPdu invariants"
        )]
...
impl<'de> Decode<'de> for GfxPdu {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let _payload_length = pdu_length
            ...
            .ok_or_else(|| invalid_field_err!("Length", "GFX PDU length is too small", in: src))?;
        ...
        match cmdid {
            ...
            _ => Err(invalid_field_err!("Type", "Unknown GFX PDU type", in: src)),
        }
    }
}
...
impl<'a> Decode<'a> for ResetGraphicsPdu {
    fn decode(src: &mut ReadCursor<'a>) -> DecodeResult<Self> {
        ...
        if width > MAX_RESET_GRAPHICS_WIDTH_HEIGHT {
            return Err(invalid_field_err!("width", "invalid reset graphics width", in: src));
        }

        let height = src.read_u32();
        if height > MAX_RESET_GRAPHICS_WIDTH_HEIGHT {
            return Err(invalid_field_err!("height", "invalid reset graphics height", in: src));
        }
