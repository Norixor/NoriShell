...
mod openh264_impl {
    ...
    fn checked_rgba_layout(width: usize, height: usize) -> DecoderResult<(u32, u32, u32, usize)> {
        // Bound decoded H.264 output before allocating its RGBA copy. EGFX may
        // report a smaller destination rectangle than the decoded bitstream.
        const MAX_DIMENSION: usize = 8_192;
        const MAX_PIXELS: usize = 16_777_216;
        if width == 0 || height == 0 || width > MAX_DIMENSION || height > MAX_DIMENSION {
            return Err(DecoderError::msg(
                "decoded H.264 frame exceeds application limit",
            ));
        }
        let pixels = width
            .checked_mul(height)
            .ok_or_else(|| DecoderError::msg("decoded H.264 frame dimensions overflow"))?;
        ...
        let rgba_stride = width
            .checked_mul(4)
            .ok_or_else(|| DecoderError::msg("decoded H.264 RGBA stride overflows"))?;
        let rgba_size = pixels
            .checked_mul(4)
            .ok_or_else(|| DecoderError::msg("decoded H.264 RGBA byte length overflows"))?;
        let (width, height, rgba_stride) = (
            u32::try_from(width).map_err(|_| DecoderError::msg("decoded H.264 width overflows"))?,
            u32::try_from(height)
                .map_err(|_| DecoderError::msg("decoded H.264 height overflows"))?,
            u32::try_from(rgba_stride)
                .map_err(|_| DecoderError::msg("decoded H.264 RGBA stride overflows"))?,
        );
        Ok((width, height, rgba_stride, rgba_size))
    }
    ...
    impl H264Decoder for OpenH264Decoder {
        fn decode(&mut self, data: &[u8]) -> DecoderResult<DecodedFrame> {
            ...
            let (w32, h32, rgba_stride, rgba_size) = checked_rgba_layout(width, height)?;
            ...
            let [y_stride, u_stride, v_stride] =
                [y_stride as u32, u_stride as u32, v_stride as u32];
    ...
    #[cfg(test)]
    mod tests {
        use super::checked_rgba_layout;

        #[test]
        fn rejects_oversized_h264_output_before_rgba_allocation() {
            assert_eq!(
                checked_rgba_layout(8_192, 2_048).unwrap(),
                (8_192, 2_048, 32_768, 67_108_864)
            );
            for (width, height) in [
                (8_193, 1),
                (1, 8_193),
                ...
                assert!(
                    checked_rgba_layout(width, height).is_err(),
