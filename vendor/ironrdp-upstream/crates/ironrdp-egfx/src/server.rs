...
use ironrdp_graphics::zgfx::{
    CompressionMode, Compressor, compress_and_wrap_egfx, wrap_uncompressed,
};
...
use crate::pdu::{
    ...
    CapabilitiesAdvertisePdu, CapabilitiesConfirmPdu, CapabilitiesV8Flags, CapabilitiesV10Flags,
    CapabilitiesV81Flags, CapabilitiesV103Flags, CapabilitiesV104Flags, CapabilitiesV107Flags,
    CapabilitySet, Codec1Type, Codec2Type, CreateSurfacePdu, DeleteSurfacePdu, Encoding,
    EndFramePdu, FrameAcknowledgePdu, GfxPdu, MapSurfaceToOutputPdu, PixelFormat,
    QoeFrameAcknowledgePdu, ResetGraphicsPdu, StartFramePdu, Timestamp, WireToSurface1Pdu,
...
fn intersect_flags(client: &CapabilitySet, server: &CapabilitySet) -> CapabilitySet {
    match (client, server) {
        (CapabilitySet::V8 { flags: cf }, CapabilitySet::V8 { flags: sf }) => {
            CapabilitySet::V8 { flags: *cf & *sf }
        }
...
impl GraphicsPipelineServer {
    ...
    pub fn send_avc420_frame(
        ...
        self.output_queue
            .push_back(GfxPdu::StartFrame(StartFramePdu {
                timestamp,
                frame_id,
            }));

        self.output_queue
            .push_back(GfxPdu::WireToSurface1(WireToSurface1Pdu {
                surface_id,
        ...
        self.output_queue
            .push_back(GfxPdu::EndFrame(EndFramePdu { frame_id }));
    ...
    fn send_avc444_frame_with_encoding(
        ...
        let stream1_rectangles: Vec<_> = stream1_regions
            .iter()
            .map(Avc420Region::to_rectangle)
            .collect();
        let stream1_quant_vals: Vec<_> = stream1_regions
            .iter()
            .map(Avc420Region::to_quant_quality)
            .collect();
        ...
        let stream2 = if encoding == Encoding::LUMA_AND_CHROMA {
            ...
            let stream2_rectangles: Vec<_> = stream2_regions
                .iter()
                .map(Avc420Region::to_rectangle)
                .collect();
            let stream2_quant_vals: Vec<_> = stream2_regions
                .iter()
                .map(Avc420Region::to_quant_quality)
                .collect();
        ...
        let target_rect = if let Some(stream2_regions) = stream2_regions {
            Self::compute_dest_rect_for_streams(
                stream1_regions,
                stream2_regions,
                surface.width,
                surface.height,
    ...
    pub fn send_remotefx_progressive_frame(
        ...
        self.output_queue
            .push_back(GfxPdu::WireToSurface2(WireToSurface2Pdu {
                surface_id,
        ...
        let messages: Vec<DvcMessage> = pdus
            ...
            .map(|pdu| {
                ...
                pdu.encode(&mut cursor)
                    .expect("GfxPdu encoding should not fail");
    ...
    fn handle_capabilities_advertise(&mut self, pdu: CapabilitiesAdvertisePdu) {
        ...
        self.output_queue.push_back(GfxPdu::CapabilitiesConfirm(
            CapabilitiesConfirmPdu::from_typed(negotiated),
        ));
    ...
    fn handle_frame_acknowledge(&mut self, pdu: FrameAcknowledgePdu) {
        ...
        if let Some(info) = self.frames.acknowledge(pdu.frame_id, queue_depth) {
            ...
            debug!(
                ...
                queue_depth,
                suspended,
                "EGFX FrameAcknowledge for an untracked issued frame (resume or stale ack)"
            );
        } else {
    ...
    fn handle_cache_import_offer(&mut self, pdu: CacheImportOfferPdu) {
        ...
        self.output_queue
            .push_back(GfxPdu::CacheImportReply(CacheImportReplyPdu {
                cache_slots: accepted,
            }));
    }
}
    ...
    fn removes_avc_thin_client_from_malformed_avc_disabled_capabilities() {
        let cases = [
            ...
            (
                ...
                CapabilitySet::V10_7 {
                    flags: CapabilitiesV107Flags::AVC_THIN_CLIENT
                        | CapabilitiesV107Flags::SMALL_CACHE,
                },
                CapabilitySet::V10_7 {
                    flags: CapabilitiesV107Flags::AVC_DISABLED | CapabilitiesV107Flags::SMALL_CACHE,
...
mod tests {
    ...
    fn mid_tier_only_client_negotiates_its_version() {
        ...
        let negotiated = negotiate_capabilities(&client_caps, &server_caps)
            .expect("V10.6-only client negotiates");
