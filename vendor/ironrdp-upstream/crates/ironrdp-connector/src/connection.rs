...
use ironrdp_svc::{
    MAX_STATIC_CHANNELS, StaticChannelKey, StaticChannelSet, StaticVirtualChannel,
    SvcClientProcessor,
};
...
use crate::{
    Config, ConnectorError, ConnectorErrorExt as _, ConnectorErrorKind, ConnectorResult,
    DesktopSize, MonotonicInstant, NegotiationFailure, Sequence, State, Written,
...
impl ClientConnector {
    ...
    pub fn initiate_with_security_protocol(
        ...
        if !matches!(
            ...
            return Err(reason_err!(
                "Initiation",
                "connection initiation has already started"
            ));
        }
    ...
    fn enabled_security_protocols(&self) -> ConnectorResult<nego::SecurityProtocol> {
        ...
        if self.config.enable_credssp {
            ...
            security_protocol
                .insert(nego::SecurityProtocol::HYBRID | nego::SecurityProtocol::HYBRID_EX);
        }
        ...
        T: SvcClientProcessor + 'static,
    {
        ...
        if self.static_channels.get_by_type::<T>().is_none()
            && self.static_channels.len() >= MAX_STATIC_CHANNELS
        {
            warn!(
                max_channels = MAX_STATIC_CHANNELS,
                "Static channel limit reached"
            );
            return;
        }
    ...
    /// registered.
    pub fn try_attach_dynamic_static_channel<T>(
        &mut self,
        channel: T,
    ) -> Result<(), DynamicStaticChannelAttachError>
    where
        T: SvcClientProcessor + 'static,
    {
        let channel_name = channel.channel_name();
        ...
        if self
            .static_channels
            .get_by_channel_name_key(&channel_name)
            .is_some()
        {
    ...
    fn multitransport_response_channel(
        &self,
        result: &MultitransportResult,
    ) -> ConnectorResult<Option<u16>> {
    ...
    fn respond_to_multitransport(
        ...
        let total_written = if let Some(response_channel) = response_channel {
            let response = result.response_pdu(request_id, soft_sync).ok_or_else(|| {
                general_err!("multitransport response channel selected without a required response")
            })?;
    ...
    fn respond_to_connect_time_autodetect(
        ...
        use ironrdp_pdu::rdp::autodetect::{
            ...
            BW_START_CONNECT_TIME, BW_STOP_CONNECT_TIME,
        };
        ...
        match request {
            AutoDetectRequest::RttRequest {
                ...
                let response =
                    AutoDetectRspPdu::new(AutoDetectResponse::RttResponse { sequence_number });
            ...
            AutoDetectRequest::BandwidthMeasureStart { request_type, .. }
                if request_type == BW_START_CONNECT_TIME =>
            {
            ...
            AutoDetectRequest::BandwidthMeasureStop {
                ...
                let (time_delta_ms, byte_count) = match (
                    self.connect_time_bw_started_at,
                    received_at,
                ) {
                    (Some(started_at), Some(stopped_at)) => {
                        let measured_ms =
                            u32::try_from(stopped_at.duration_since(started_at).as_millis())
                                .unwrap_or(u32::MAX);
                        (
            ...
            AutoDetectRequest::BandwidthMeasureStop {
                ...
                warn!(
                    sequence_number,
                    request_type,
                    "Unexpected Bandwidth Measure Stop requestType during connect-time auto-detection"
                );
                Ok(Written::Nothing)
            }
    ...
    fn step(
        ...
        let (written, next_state) = match mem::take(&mut self.state) {
            ...
            ClientConnectorState::BasicSettingsExchangeSendInitial { selected_protocol } => {
                ...
                debug!("Send initial connection settings");
            ...
            ClientConnectorState::BasicSettingsExchangeWaitResponse { connect_initial } => {
                ...
                zipped.into_iter().for_each(|(channel, channel_id)| {
                    self.static_channels
                        .attach_channel_id_by_key(channel, channel_id);
                });
            ...
            ClientConnectorState::SecureSettingsExchange {
                ...
                // Client info contains account and endpoint metadata; keep it out of logs.
            ...
            ClientConnectorState::ConnectTimeAutoDetection {
                ...
                let message_channel_pdu = self.message_channel_id.and_then(|message_channel_id| {
                    ...
                    match mcs.0 {
                        mcs::McsMessage::SendDataIndication(data)
                ...
                if let Some((message_channel_id, data)) = message_channel_pdu {
                    if let Ok(autodetect) =
                        decode::<rdp::autodetect::AutoDetectReqPdu>(&data.user_data)
                    {
            ...
            ClientConnectorState::MultitransportBootstrapping {
                ...
                if Some(ctx.channel_id) == message_channel_id {
                    let pdu =
                        decode::<rdp::multitransport::MultitransportRequestPdu>(ctx.user_data)
                            .map_err(ConnectorError::decode)?;
                    ...
                    (
                        ...
                        match connection_activation.connection_activation_state() {
                            ...
                            _ => ClientConnectorState::CapabilitiesExchange {
                                connection_activation,
                            },
                        },
                    )
            ...
            ClientConnectorState::ConnectionFinalization {
                ...
                let next_state = if !connection_activation
...
fn create_gcc_blocks<'a>(
    ...
    use ironrdp_pdu::gcc::{
        ...
        ClientNetworkData, ClientSecurityData, ColorDepth, EncryptionMethod, HighColorDepth,
...
fn create_client_info_pdu(
    ...
    use ironrdp_pdu::rdp::client_info::{
        AddressFamily, ClientAutoReconnect, ClientInfo, ClientInfoFlags, CompressionType,
        Credentials, ExtendedClientInfo, ExtendedClientOptionalInfo,
    };
        ...
        extra_info: ExtendedClientInfo {
            ...
            optional_data: {
                ...
                match auto_reconnect_cookie {
                    Some(cookie) => builder
                        .reconnect_cookie(
                            ClientAutoReconnect::from_server_cookie(cookie).to_bytes(),
                        )
                        .build(),
                    None => builder.build(),
                }
...
mod tests {
    ...
    fn remote_application_client_info_uses_rail_launch_data() {
        ...
        let client_info =
            create_client_info_pdu(&config, &"127.0.0.1:3389".parse().unwrap(), None).client_info;
    ...
    fn audio_capture_flag_is_set_when_enabled() {
        ...
        assert!(
            !client_info
                .flags
                .contains(ClientInfoFlags::NO_AUDIO_PLAYBACK)
        );
