...
use crate::{
    Config, ConnectionFinalizationSequence, ConnectorError, ConnectorErrorExt as _,
    ConnectorResult, DesktopSize, MonotonicInstant, Sequence, State, Written, general_err,
    reason_err,
};
    ...
    fn step(
        ...
        let (written, next_state) = match mem::take(&mut self.state) {
            ...
            ConnectionActivationState::CapabilitiesExchange => {
                ...
                let send_data_indication_ctx = ironrdp_pdu::mcs::decode_send_data_indication(input)
                    .map_err(ConnectorError::decode)?;
                let share_control_ctx =
                    rdp::headers::decode_share_control(send_data_indication_ctx)
                        .map_err(ConnectorError::decode)?;
                ...
                ) = share_control_ctx.pdu
                {
                    ...
                    if let rdp::headers::ShareControlPdu::Data(rdp::headers::ShareDataHeader {
                        share_data_pdu:
                            rdp::headers::ShareDataPdu::ServerSetErrorInfo(
                ...
                let (refresh_rect_support, suppress_output_support) = capability_sets
                    ...
                    .find_map(|capability_set| {
                        ...
                        if general.protocol_version != rdp::capability_sets::PROTOCOL_VER {
                            warn!(
                                version = general.protocol_version,
...
fn negotiated_window_support_level(window_list: Option<&WindowList>) -> Option<WindowSupportLevel> {
    window_list.and_then(|window_list| {
        (window_list.support_level != WindowSupportLevel::NotSupported)
            .then_some(window_list.support_level)
    })
}
...
fn remote_app_rail_capability(
    ...
    let window_list_supported = window_list
        .is_some_and(|window_list| window_list.support_level != WindowSupportLevel::NotSupported);
...
fn create_client_confirm_active(
    ...
    use ironrdp_pdu::rdp::capability_sets::{
        ...
        General, GeneralExtraFlags, GlyphCache, GlyphSupportLevel, Input, LargePointer,
        ...
        SurfaceCommands, VirtualChannel, VirtualChannelFlags,
    };
    ...
    // Recompute surface negotiation for every Demand Active, including reactivation.
    // RemoteFX needs fast-path output and Stream Surface Bits; do not advertise it
    // when a server withdraws either capability.
    ...
    let mut bitmap_codecs = config.bitmap.as_ref().map(|bitmap| bitmap.codecs.clone());
    if let Some(codecs) = &mut bitmap_codecs {
        codecs.0.retain(|codec| match &codec.property {
            rdp::capability_sets::CodecProperty::RemoteFx(_) => {
                surface_flags.contains(CmdFlags::STREAM_SURFACE_BITS)
            }
    ...
    if !surface_flags.is_empty() {
        server_capability_sets.push(CapabilitySet::SurfaceCommands(SurfaceCommands {
            flags: surface_flags,
        }));
    }
    if let Some(codecs) = bitmap_codecs.filter(|codecs| !codecs.0.is_empty()) {
...
mod tests {
    ...
    use ironrdp_pdu::rdp::capability_sets::{
        BitmapCodecs, CapabilitySet, CmdFlags, Codec, CodecProperty, DemandActive,
        FrameAcknowledge, General, GeneralExtraFlags, Rail, RailSupportLevel, ServerDemandActive,
        SurfaceCommands, WindowList, WindowSupportLevel, client_codecs_capabilities,
    };
    use ironrdp_pdu::rdp::headers::{ShareControlHeader, ShareControlPdu};
    use ironrdp_pdu::{gcc, mcs, x224::X224};

    use super::{
        ConnectionActivationFactory, ConnectionActivationSequence, create_client_confirm_active,
        negotiated_window_support_level, remote_app_rail_capability, requested_bitmap_color_depth,
        server_window_list,
    };
    use crate::{BitmapConfig, Config, Credentials, DesktopSize, Sequence as _};
    ...
    fn remote_app_capabilities_require_server_rail_and_window_list() {
        ...
        assert!(
            remote_app_rail_capability(true, rail_support_level, &[], Some(&window_list)).is_err()
        );
        assert!(
            remote_app_rail_capability(
                true,
    ...
    fn confirm_active_advertises_surface_capabilities_with_bitmap_codecs() {
        ...
        let confirm = create_client_confirm_active(
            ...
            offered(
                true,
                Some(CmdFlags::SET_SURFACE_BITS | CmdFlags::FRAME_MARKER),
                true,
            ),
            DesktopSize {
                width: 1024,
    ...
    fn exchange(
        ...
        sequence.step(&packet, None, &mut output).unwrap();
    ...
    #[test]
    fn reactivation_recomputes_surface_capabilities_from_each_demand_active() {
        let bitmap = BitmapConfig {
            color_depth: 32,
            lossy_compression: false,
            codecs: client_codecs_capabilities(&[]).unwrap(),
        };
        let config = test_config(Some(bitmap));
        let factory = ConnectionActivationFactory::new(config.clone(), 1003, 1002);
        let all =
            CmdFlags::SET_SURFACE_BITS | CmdFlags::STREAM_SURFACE_BITS | CmdFlags::FRAME_MARKER;
        let initial = exchange(
            &mut ConnectionActivationSequence::new(config, 1003, 1002),
            offered(true, Some(all), true),
        );
        assert!(initial.iter().any(|cap| matches!(cap, CapabilitySet::BitmapCodecs(codecs) if codecs.0.iter().any(|codec| matches!(codec.property, CodecProperty::RemoteFx(_))))));
        assert!(
            initial
                .iter()
                .any(|cap| matches!(cap, CapabilitySet::FrameAcknowledge(_)))
        );
        for withdrawn in [
            offered(false, Some(all), true),
            offered(true, None, true),
            Vec::new(),
        ] {
            let caps = exchange(&mut factory.create(), withdrawn);
        ...
        assert!(caps.iter().any(
            |cap| matches!(cap, CapabilitySet::SurfaceCommands(surface) if surface.flags == CmdFlags::SET_SURFACE_BITS)
        ));
        assert!(!caps.iter().any(|cap| matches!(
            cap,
            CapabilitySet::BitmapCodecs(_) | CapabilitySet::FrameAcknowledge(_)
        )));
        let restored = exchange(&mut factory.create(), offered(true, Some(all), true));
        assert!(
            restored
                .iter()
                .any(|cap| matches!(cap, CapabilitySet::BitmapCodecs(_)))
        );
    }
