...
impl<'de> Decode<'de> for General {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if compression_types != 0 {
            return Err(
                invalid_field_err!("compressionTypes", "invalid compression types", in: src),
            );
        }
        ...
        if remote_unshare_flag != 0 {
            return Err(
                invalid_field_err!("remoteUnshareFlags", "invalid remote unshare flag", in: src),
            );
        }
        ...
        if compression_level != 0 {
            return Err(
                invalid_field_err!("compressionLevel", "invalid compression level", in: src),
            );
        }
