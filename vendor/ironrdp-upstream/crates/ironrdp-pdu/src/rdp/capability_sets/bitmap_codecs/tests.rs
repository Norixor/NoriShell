...
static RFX_CLIENT_CAPS_CONTAINER: LazyLock<RfxClientCapsContainer> =
    LazyLock::new(|| RfxClientCapsContainer {
...
fn buffer_length_is_correct_for_rfx_client_caps_container() {
    assert_eq!(
        ...
        RFX_CLIENT_CAPS_CONTAINER.size()
    );
}
...
fn from_buffer_correctly_parses_codec_server_mode() {
    assert_eq!(
        *CODEC_SERVER_MODE,
        decode(CODEC_SERVER_MODE_BUFFER.as_ref()).unwrap()
    );
}
...
fn from_buffer_correctly_parses_bitmap_codecs() {
    assert_eq!(
        *BITMAP_CODECS,
        decode(BITMAP_CODECS_BUFFER.as_ref()).unwrap()
    );
}
