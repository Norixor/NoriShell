    ...
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let property = match guid {
            ...
            GUID_REMOTEFX | GUID_IMAGE_REMOTEFX => {
                let byte = property_buffer.first().ok_or_else(
                    || invalid_field_err!("remotefx property", "must not be empty", in: src),
                )?;
                ...
                match guid {
                    ...
                    _ => unreachable!(
                        "guid validated as RemoteFX or ImageRemoteFX by the outer match"
                    ),
                }
            }
...
impl<'de> Decode<'de> for RfxICap {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if color_conversion != RFX_ICAP_COLOR_CONVERSION {
            return Err(
                invalid_field_err!("colorConv", "invalid rfx color conversion bits", in: src),
            );
        }
        ...
        let entropy_bits = EntropyBits::from_u8(src.read_u8()).ok_or_else(
            || invalid_field_err!("entropyBits", "invalid rfx entropy bits", in: src),
        )?;
