use ironrdp_core::{
    ...
    ensure_fixed_part_size, ensure_size, invalid_field_err, unsupported_value_err, write_padding,
};
...
pub use self::bitmap_codecs::{
    BitmapCodecs, CODEC_ID_NONE, CODEC_ID_QOI, CODEC_ID_QOIZ, CODEC_ID_REMOTEFX, CaptureFlags,
    Codec, CodecId, CodecProperty, EntropyBits, Guid, NsCodec, RemoteFxContainer, RfxCaps,
    RfxCapset, RfxClientCapsContainer, RfxICap, RfxICapFlags, client_codecs_capabilities,
    server_codecs_capabilities,
};
        ...
        match self {
            ...
            _ => {
                let (capability_set_type, capability_set_buffer) = match self {
                    ...
                    CapabilitySet::BitmapCacheV3(buffer) => {
                        (CapabilitySetType::BitmapCacheV3CodecID, buffer)
                    }
        ...
        match capability_set_type {
            ...
            CapabilitySetType::Rail => {
                if buffer_length != Rail::FIXED_PART_SIZE {
                    return Err(invalid_field_err!(
                        "len",
                        "invalid RAIL capability set length"
                    ));
                }
            ...
            CapabilitySetType::WindowList => {
                if buffer_length != WindowList::FIXED_PART_SIZE {
                    return Err(invalid_field_err!(
                        "len",
                        "invalid window list capability set length"
                    ));
                }
