...
use ironrdp_core::{
    Decode, DecodeResult, Encode, EncodeResult, ReadCursor, WriteCursor, ensure_size,
    invalid_field_err,
};
    ...
    fn size(&self) -> usize {
        match self {
            ...
            Self::NetworkCharacteristicsResult { request_type, .. } => {
                let (want_base_rtt, want_bandwidth) = netchar_fields(*request_type);
                HEADER_MIN_SIZE
                    + if want_base_rtt { 4 } else { 0 }
                    + if want_bandwidth { 4 } else { 0 }
                    + 4 /* averageRTT */
            }
        }
    }
...
impl<'de> Decode<'de> for AutoDetectRequest {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        match request_type {
            ...
            BW_START_CONNECT_TIME | BW_START_RELIABLE_UDP | BW_START_LOSSY_UDP => {
                Ok(Self::BandwidthMeasureStart {
                    sequence_number,
                    request_type,
                })
            }
...
impl<'de> Decode<'de> for AutoDetectResponse {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        match response_type {
            ...
            _ => {
                Err(invalid_field_err!("responseType", "unknown autodetect response type", in: src))
            }
        }
    }
}
...
impl<'de> Decode<'de> for AutoDetectReqPdu {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if !security_header
            .flags
            .contains(BasicSecurityHeaderFlags::AUTODETECT_REQ)
        {
            return Err(
                invalid_field_err!("securityHeader", "expected SEC_AUTODETECT_REQ flag", in: src),
            );
        }
...
impl<'de> Decode<'de> for AutoDetectRspPdu {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if !security_header
            .flags
            .contains(BasicSecurityHeaderFlags::AUTODETECT_RSP)
        {
            return Err(
                invalid_field_err!("securityHeader", "expected SEC_AUTODETECT_RSP flag", in: src),
            );
        }
...
mod tests {
    ...
    fn req_pdu_round_trip() {
        ...
        assert_eq!(
            original.security_header.flags,
            BasicSecurityHeaderFlags::AUTODETECT_REQ
        );
    ...
    fn rsp_pdu_round_trip() {
        let original =
            AutoDetectRspPdu::new(AutoDetectResponse::RttResponse { sequence_number: 7 });
        assert_eq!(
            original.security_header.flags,
            BasicSecurityHeaderFlags::AUTODETECT_RSP
        );
