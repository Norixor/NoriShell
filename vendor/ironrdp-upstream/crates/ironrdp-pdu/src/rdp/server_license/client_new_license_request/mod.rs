...
use ironrdp_core::{
    Decode as _, DecodeResult, Encode as _, EncodeResult, ReadCursor, WriteCursor, ensure_size,
    invalid_field_err,
};
use md5::Digest as _;
...
use super::{
    BasicSecurityHeader, BasicSecurityHeaderFlags, BlobHeader, BlobType,
    KEY_EXCHANGE_ALGORITHM_RSA, LicenseEncryptionData, LicenseHeader, PREAMBLE_SIZE, PreambleFlags,
    PreambleType, PreambleVersion, RANDOM_NUMBER_SIZE, ServerLicenseError, ServerLicenseRequest,
...
impl ClientNewLicenseRequest {
    ...
    pub fn decode(license_header: LicenseHeader, src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ...
        if key_exchange_algorithm != KEY_EXCHANGE_ALGORITHM_RSA {
            return Err(
                invalid_field_err!("keyExchangeAlgo", "invalid key exchange algorithm", in: src),
            );
        }
