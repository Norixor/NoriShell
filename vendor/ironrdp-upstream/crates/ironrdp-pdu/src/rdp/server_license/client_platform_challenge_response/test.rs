...
const CLIENT_PLATFORM_CHALLENGE_RESPONSE_BUFFER: [u8; 70] = [
    ...
    0x12, 0x00, // blob len
    0xfa, 0xb4, 0xe8, 0x24, 0xcf, 0x56, 0xb2, 0x4e, 0x80, 0x02, 0xbd, 0xb6, 0x61, 0xfc, 0xdf, 0xe9,
    0x6c, 0x44, // encrypted platform challenge response
    0x09, 0x00, // blob type, ignored
    0x14, 0x00, // blob len
    0xf8, 0xb5, 0xe8, 0x25, 0x3d, 0x0f, 0x3f, 0x70, 0x1d, 0xda, 0x60, 0x19, 0x16, 0xfe, 0x73, 0x1a,
    0x45, 0x7e, 0x02, 0x71, // encrypted hwid
...
pub(crate) static RESPONSE: LazyLock<PlatformChallengeResponseData> =
    LazyLock::new(|| PlatformChallengeResponseData {
...
pub(crate) static CLIENT_PLATFORM_CHALLENGE_RESPONSE: LazyLock<LicensePdu> = LazyLock::new(|| {
    LicensePdu::ClientPlatformChallengeResponse(ClientPlatformChallengeResponse {
        ...
        encrypted_challenge_response_data: Vec::from(
...
fn challenge_response_creates_from_server_challenge_and_encryption_data_correctly() {
    ...
    let mac_data = vec![
        0x51, 0x4a, 0x27, 0x2c, 0x74, 0x18, 0xec, 0x88, 0x95, 0xdd, 0xac, 0x10, 0x3e, 0x3f, 0xa,
        0x72,
    ];
    ...
    let server_challenge = ServerPlatformChallenge {
        license_header: LicenseHeader {
            ...
            preamble_version: PreambleVersion::V3,
            preamble_message_size: u16::try_from(
                encrypted_platform_challenge.len() + mac_data.len() + PREAMBLE_SIZE,
            )
            .expect("can't panic"),
        },
    ...
    let encryption_data = LicenseEncryptionData {
        ...
        mac_salt_key: vec![
            0x1, 0x5b, 0x9e, 0x5f, 0x6, 0x97, 0x71, 0x58, 0xc3, 0xb8, 0x8b, 0x8c, 0x6e, 0x77, 0x21,
            0x37,
        ],
        license_key: vec![
            0xe1, 0x78, 0xe4, 0xa0, 0x2a, 0xc5, 0xca, 0xb8, 0xa2, 0xd1, 0x53, 0xb8, 0x7, 0x23,
            0xf3, 0xd2,
        ],
    };
    ...
    let response_data: [u8; 26] = [
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0xff, 0x03, 0x00, 0x0a,
    ...
    let mac_data = crate::rdp::server_license::compute_mac_data(
        ...
        [response_data.as_ref(), hardware_id.as_slice()]
            .concat()
            .as_slice(),
    )
    .expect("can't panic");
