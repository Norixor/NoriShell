...
use super::{
    BLOB_LENGTH_SIZE, BLOB_TYPE_SIZE, BasicSecurityHeader, BasicSecurityHeaderFlags, BlobHeader,
    BlobType, LicenseEncryptionData, LicenseHeader, MAC_SIZE, PLATFORM_ID, PREAMBLE_SIZE,
    PreambleFlags, PreambleType, PreambleVersion, ServerLicenseError, ServerPlatformChallenge,
};
...
impl<'de> Decode<'de> for PlatformChallengeResponseData {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if version != RESPONSE_DATA_VERSION {
            return Err(
                invalid_field_err!("version", "invalid challenge response version", in: src),
            );
        }
        ...
        let license_detail_level = LicenseDetailLevel::from_u16(src.read_u16()).ok_or_else(
            || invalid_field_err!("licenseDetailLevel", "invalid license detail level", in: src),
        )?;
