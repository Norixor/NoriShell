...
use super::{
    BLOB_LENGTH_SIZE, BLOB_TYPE_SIZE, BlobHeader, BlobType, KEY_EXCHANGE_ALGORITHM_RSA,
    LicenseHeader, PreambleType, RANDOM_NUMBER_SIZE, ServerLicenseError, UTF8_NULL_TERMINATOR_SIZE,
...
impl<'de> Decode<'de> for ServerCertificate {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let certificate = match cert_version & CERT_CHAIN_VERSION_MASK {
            ...
            _ => {
                return Err(
                    invalid_field_err!("certVersion", "invalid certificate version", in: src),
                );
            }
        };
