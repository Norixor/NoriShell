...
use ironrdp_core::{
    ...
    ensure_fixed_part_size, invalid_field_err, unsupported_value_err,
};
use md5::Digest as _;
...
use crate::rdp::headers::{
    BASIC_SECURITY_HEADER_SIZE, BasicSecurityHeader, BasicSecurityHeaderFlags,
};
...
pub use self::server_license_request::{
    ProductInfo, Scope, ServerCertificate, ServerLicenseRequest, cert,
};
...
impl<'de> Decode<'de> for LicenseHeader {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let preamble_flags = PreambleFlags::from_bits(flags_with_version & !PROTOCOL_VERSION_MASK)
            .ok_or_else(
                || invalid_field_err!("preambleFlags", "Got invalid flags field", in: src),
            )?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidField(name) => {
                write!(f, "invalid `{name}`: out of range integral type conversion")
            }
            ...
            Self::RandomNumberGenerationError(s) => {
                write!(f, "unable to generate random number {s}")
            }
            Self::UnableToGetPublicKey => {
                f.write_str("unable to retrieve public key from the certificate")
            }
            Self::RsaKeyEncryptionError => f.write_str("unable to encrypt RSA public key"),
            Self::InvalidKeyExchangeValue => {
                f.write_str("invalid License Request key exchange algorithm value")
            }
            Self::InvalidMacData => f.write_str(
                "MAC checksum generated over decrypted data does not match the server's checksum",
            ),
            ...
            Self::InvalidPropCertKeyAlgorithmId => {
                f.write_str("invalid proprietary certificate key algorithm ID")
            }
            ...
            Self::InvalidRsaPublicKeyDataLength => {
                f.write_str("invalid RSA public key data length")
            }
            ...
            Self::UnexpectedError(msg) => {
                write!(f, "the server returned unexpected error: {msg:?}")
            }
            Self::UnexpectedLicenseMessage => f.write_str("got unexpected license message"),
            Self::UnexpectedServerError(_) => {
                f.write_str("the server has returned an unexpected error")
            }
            Self::ValidClientStatus(_) => {
                f.write_str("the server has returned STATUS_VALID_CLIENT (not an error)")
            }
            Self::InvalidKeyExchangeAlgorithm => f.write_str("invalid Key Exchange List field"),
            Self::InvalidCompanyNameLength(n) => {
                write!(
                    f,
                    "received invalid company name length (Product Information): {n}"
                )
            }
            Self::InvalidProductIdLength(n) => {
                write!(
                    f,
                    "received invalid product ID length (Product Information): {n}"
                )
            }
            Self::InvalidScopeCount(n) => write!(f, "received invalid scope count field: {n}"),
            Self::InvalidCertificateLength(n) => {
                write!(f, "received invalid certificate length: {n}")
            }
