...
use ironrdp_core::{
    Decode as _, DecodeResult, Encode as _, EncodeResult, ReadCursor, WriteCursor, cast_length,
...
use super::{
    BLOB_LENGTH_SIZE, BLOB_TYPE_SIZE, BlobHeader, BlobType, LicenseHeader, PreambleFlags,
    PreambleVersion,
};
...
impl LicensingErrorMessage {
    ...
    pub fn decode(license_header: LicenseHeader, src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        if license_header.preamble_message_type != PreambleType::ErrorAlert {
            return Err(
                invalid_field_err!("preambleMessageType", "unexpected preamble type", in: src),
            );
        }

        ensure_fixed_part_size!(in: src);
        let error_code = LicenseErrorCode::from_u32(src.read_u32())
            .ok_or_else(|| invalid_field_err!("errorCode", "invalid error code", in: src))?;
        let state_transition = LicensingStateTransition::from_u32(src.read_u32()).ok_or_else(
            || invalid_field_err!("stateTransition", "invalid state transition", in: src),
        )?;
