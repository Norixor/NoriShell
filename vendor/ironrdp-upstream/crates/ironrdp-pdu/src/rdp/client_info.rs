...
impl<'de> Decode<'de> for ClientInfo {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        // value.
        let flags =
            ClientInfoFlags::from_bits_retain(flags_with_compression_type & !COMPRESSION_TYPE_MASK);
        ...
        let alternate_shell =
            utils::decode_string(src.read_slice(alternate_shell_size), character_set, true)?;
...
impl ExtendedClientInfo {
    ...
    fn encode(&self, dst: &mut WriteCursor<'_>, character_set: CharacterSet) -> EncodeResult<()> {
        ...
        let address_string_len: u16 = cast_length!("address length", string_len(self.address.as_str(), character_set), in: dst)?;
        let dir_string_len: u16 =
            cast_length!("dir length", string_len(self.dir.as_str(), character_set), in: dst)?;
...
impl ClientAutoReconnect {
    ...
    pub fn verify(&self, cookie: &crate::rdp::session_info::ServerAutoReconnect) -> bool {
        ...
        self.logon_id == cookie.logon_id
            && Self::keyed_hmac(cookie)
                .verify_slice(&self.security_verifier)
                .is_ok()
    }
...
impl<'de> Decode<'de> for ClientAutoReconnect {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if version != RECONNECT_COOKIE_VERSION {
            return Err(
                invalid_field_err!("Version", "invalid auto-reconnect version", at: version_pos),
            );
        }
...
impl ExtendedClientOptionalInfo {
    ...
    > {
        builder::ExtendedClientOptionalInfoBuilder::<
            builder::ExtendedClientOptionalInfoBuilderStateSetTimeZone,
        >::default()
    }
...
impl Encode for ExtendedClientOptionalInfo {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        if let Some(reconnect_cookie) = self.reconnect_cookie {
            dst.write_u16(
                u16::try_from(RECONNECT_COOKIE_LEN).expect("RECONNECT_COOKIE_LEN fit into u16"),
            );
...
impl<'de> Decode<'de> for ExtendedClientOptionalInfo {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        optional_data.performance_flags =
            Some(PerformanceFlags::from_bits(src.read_u32()).ok_or_else(
                || invalid_field_err!("performanceFlags", "invalid performance flags", in: src),
            )?);
        ...
        if reconnect_cookie_size
            != u16::try_from(RECONNECT_COOKIE_LEN).expect("RECONNECT_COOKIE_LEN fit into u16")
            && reconnect_cookie_size != 0
        {
            return Err(
                invalid_field_err!("cbAutoReconnectCookie", "invalid cookie size", in: src),
            );
        }
        if reconnect_cookie_size != 0 {
            if src.len() < RECONNECT_COOKIE_LEN {
                return Err(
                    invalid_field_err!("cbAutoReconnectCookie", "missing cookie data", in: src),
                );
            }
            let reconnect_cookie = src.read_array();
            optional_data.auto_reconnect =
                ClientAutoReconnect::decode(&mut ReadCursor::new(&reconnect_cookie)).ok();
...
pub mod builder {
    ...
    use super::{
        ClientAutoReconnect, ExtendedClientOptionalInfo, PerformanceFlags, RECONNECT_COOKIE_LEN,
        TimezoneInfo,
    };
    ...
    impl Default
        for ExtendedClientOptionalInfoBuilder<ExtendedClientOptionalInfoBuilderStateSetTimeZone>
    {
        fn default() -> Self {
            Self::new()
        }
...
mod tests {
    ...
    fn security_verifier_is_redacted_from_debug_output() {
        ...
        for (label, rendered) in [
            ...
            assert!(
                ...
                "{label} leaked the verifier: {rendered}"
            );
        }
    ...
    fn malformed_auto_reconnect_packet_is_preserved_but_not_offered() {
        ...
        let decoded: ExtendedClientOptionalInfo =
            decode(&encode_vec(&optional_info).unwrap()).unwrap();
