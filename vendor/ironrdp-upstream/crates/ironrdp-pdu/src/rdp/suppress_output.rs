...
impl<'de> Decode<'de> for SuppressOutputPdu {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let allow_display_updates = AllowDisplayUpdatesType::from_u8(src.read_u8()).ok_or_else(
            || invalid_field_err!("allowDisplayUpdates", "invalid display update type", in: src),
        )?;
        read_padding!(src, 3);
