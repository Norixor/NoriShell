...
use ironrdp_core::{
    Decode, DecodeResult, Encode, EncodeResult, ReadCursor, WriteBuf, WriteCursor, cast_length,
    decode, ensure_fixed_part_size, ensure_size, invalid_field_err, not_enough_bytes_err,
    other_err, read_padding, unsupported_value_err, write_padding,
};
...
pub fn decode_io_channel(ctx: SendDataIndicationCtx<'_>) -> DecodeResult<IoChannelPdu> {
    ...
    match ctx.pdu {
        ShareControlPdu::ServerDeactivateAll(deactivate_all) => {
...
impl<'de> Decode<'de> for ShareControlHeader {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if pdu_type == ShareControlPduType::DataPdu {
            ...
            } else if total_length < header_length
                && !is_header_length_font_map
                && !is_zero_length_empty_output_pdu
            {
...
impl ShareControlPdu {
    ...
    pub fn from_type(
        ...
        match share_type {
            ...
            _ => {
                Err(invalid_field_err!("share_type", "unexpected share control PDU type", in: src))
            }
        }
    }
}
...
impl<'de> Decode<'de> for ShareDataHeader {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        read_padding!(src, 1);
        let stream_priority = StreamPriority::from_u8(src.read_u8()).ok_or_else(
            || invalid_field_err!("streamPriority", "Invalid stream priority", in: src),
        )?;
        ...
        .ok_or_else(
            || invalid_field_err!("compressionType", "Invalid compression type", in: src),
        )?;
...
impl Encode for ShareDataPdu {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        match self {
            ...
            ShareDataPdu::Compressed { .. } => Err(other_err!(
                "Encoding compressed Share Data PDU is not implemented"
            )),
...
mod tests {
    ...
    fn reject_zero_length_non_output_data_pdu() {
        ...
        let error = decode::<ShareControlHeader>(&encoded)
            .expect_err("zero total length is only accepted for no-op output");
    ...
    fn share_data_context_retains_compression_metadata() {
        ...
        user_data[COMPRESSION_CONTROL_OFFSET] =
            (CompressionFlags::COMPRESSED | CompressionFlags::FLUSHED).bits()
                | CompressionType::K64.as_u8();
        ...
        let data_ctx =
            crate::mcs::decode_send_data_indication(&frame).expect("decode SendDataIndication");
    ...
    fn data_wrapping_set_error_info_surfaces_description() {
        ...
        let pdu = wrap_in_data(ShareDataPdu::ServerSetErrorInfo(ServerSetErrorInfoPdu(
            error,
        )));
