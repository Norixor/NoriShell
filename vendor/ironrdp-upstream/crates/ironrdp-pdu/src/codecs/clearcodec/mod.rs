...
pub use self::bands::{
    Band, MAX_BAND_HEIGHT, SHORT_VBAR_CACHE_SIZE, ShortVBarCacheMiss, VBAR_CACHE_SIZE, VBar,
    decode_bands_layer,
};
...
impl<'a> CompositePayload<'a> {
    ...
    pub fn decode(src: &mut ReadCursor<'a>) -> DecodeResult<Self> {
        ...
        let residual_byte_count: usize =
            cast_length!("residualByteCount", src.read_u32(), in: src)?;
        let bands_byte_count: usize = cast_length!("bandsByteCount", src.read_u32(), in: src)?;
        let subcodec_byte_count: usize =
            cast_length!("subcodecByteCount", src.read_u32(), in: src)?;

        let total = residual_byte_count
            .checked_add(bands_byte_count)
            .and_then(|s| s.checked_add(subcodec_byte_count))
            .ok_or_else(
                || invalid_field_err!("byteCount", "layer byte counts overflow", in: src),
            )?;
