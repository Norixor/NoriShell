...
fn decode_single_subcodec<'a>(src: &mut ReadCursor<'a>) -> DecodeResult<Subcodec<'a>> {
    ...
    let bitmap_data_byte_count: usize =
        cast_length!("bitmapDataByteCount", src.read_u32(), in: src)?;
    let codec_id_raw = src.read_u8();
    let codec_id = SubcodecId::from_u8(codec_id_raw)?;

    if width == 0 || height == 0 {
        return Err(
            invalid_field_err!("dimensions", "subcodec region has zero dimension", in: src),
        );
    }
