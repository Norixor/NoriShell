...
fn decode_vbar<'a>(src: &mut ReadCursor<'a>, band_height: u16) -> DecodeResult<VBar<'a>> {
    ...
    let y_off =
        u8::try_from(first_word >> 8).expect("top 2 bits are clear, so shifted value fits in u8");

    if y_off < y_on {
        return Err(
            invalid_field_err!("shortVBarCacheMiss", "shortVBarYOff < shortVBarYOn", in: src),
        );
    }
