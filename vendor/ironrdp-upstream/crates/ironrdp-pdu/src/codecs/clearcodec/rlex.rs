...
fn decode_single_palette_segments(
...
fn decode_multi_palette_segments(
    ...
    while !src.is_empty() {
        ...
        if stop_index >= palette_count {
            return Err(
                invalid_field_err!("rlexStopIndex", "stop_index exceeds palette count", in: src),
            );
        }
        ...
        let start_index = stop_index.checked_sub(suite_depth).ok_or_else(
            || invalid_field_err!("rlexSuiteDepth", "suite depth exceeds stop index", in: src),
        )?;
