...
impl Decode<'_> for ProgressiveBlockHeader {
    fn decode(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ...
        let block_type = ProgressiveBlockType::from_u16(raw).ok_or_else(
            || invalid_field_err!("blockType", "unknown progressive block type", in: src),
        )?;
...
impl Decode<'_> for ProgressiveCodecQuant {
    #[expect(
        ...
        reason = "y/cb/cr quant names follow spec terminology"
    )]
    fn decode(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ensure_fixed_part_size!(in: src);
...
impl<'de> Decode<'de> for TileSimple<'de> {
    #[expect(
        clippy::similar_names,
        reason = "y/cb/cr quant and length names follow spec"
    )]
...
impl<'de> Decode<'de> for TileUpgrade<'de> {
    #[expect(
        clippy::similar_names,
        reason = "SRL/raw per component is inherently similar"
    )]
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ensure_size!(ctx: Self::NAME, in: src, size: Self::HEADER_SIZE);
        let quant_idx_y = src.read_u8();
...
impl Encode for ProgressiveRegion<'_> {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        for tile in &self.tiles {
            ...
            let block_len: u32 =
                cast_length!("tileBlockLen", ProgressiveBlockHeader::SIZE + body_size, in: dst)?;
            let header = ProgressiveBlockHeader {
                block_type,
                block_len,
            };
            header.encode(dst)?;
...
impl<'de> Decode<'de> for ProgressiveRegion<'de> {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if num_quant > 7 {
            return Err(
                invalid_field_err!("numQuant", "quant count exceeds maximum of 7", in: src),
            );
        }
        ...
        for _ in 0..num_tiles {
            ...
            let body_len = header
                ...
                .ok_or_else(
                    || invalid_field_err!("blockLen", "tile block length too small", in: src),
                )?;
            ...
            let tile = match header.block_type {
                ProgressiveBlockType::TileSimple => {
                    ProgressiveTile::Simple(TileSimple::decode(tile_src)?)
                }
                ProgressiveBlockType::TileFirst => {
                    ProgressiveTile::First(TileFirst::decode(tile_src)?)
                }
                ProgressiveBlockType::TileUpgrade => {
                    ProgressiveTile::Upgrade(TileUpgrade::decode(tile_src)?)
                }
                _ => {
                    return Err(
                        invalid_field_err!("blockType", "expected tile block inside region", in: src),
                    );
                }
            };
            tiles.push(tile);
        ...
        for tile in &tiles {
            ...
            if indices.iter().any(|&i| usize::from(i) >= quant_count) {
                return Err(
                    invalid_field_err!("quantIdx", "tile quant index out of range", in: src),
                );
            }
        }
...
pub fn decode_progressive_stream<'a>(data: &'a [u8]) -> DecodeResult<Vec<ProgressiveBlock<'a>>> {
    ...
    while src.len() >= ProgressiveBlockHeader::SIZE {
        ...
        if let Some(expected) = expected_body {
            if body_len != expected {
                return Err(invalid_field_err!(
                    "blockLen",
                    "unexpected size for fixed-size block"
                ));
            }
        }
        ...
        let block = match header.block_type {
            ProgressiveBlockType::Sync => {
                ProgressiveBlock::Sync(ProgressiveSyncPdu::decode(body_src)?)
            }
            ProgressiveBlockType::FrameBegin => {
                ProgressiveBlock::FrameBegin(ProgressiveFrameBeginPdu::decode(body_src)?)
            }
            ProgressiveBlockType::FrameEnd => {
                ProgressiveBlock::FrameEnd(ProgressiveFrameEndPdu::decode(body_src)?)
            }
            ProgressiveBlockType::Context => {
                ProgressiveBlock::Context(ProgressiveContextPdu::decode(body_src)?)
            }
            ProgressiveBlockType::Region => {
                ProgressiveBlock::Region(ProgressiveRegion::decode(body_src)?)
            }
...
#[expect(
    clippy::similar_names,
    reason = "y/cb/cr test variables follow spec terminology"
)]
mod tests {
    use super::*;
