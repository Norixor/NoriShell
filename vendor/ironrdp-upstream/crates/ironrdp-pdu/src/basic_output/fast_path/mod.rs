...
use ironrdp_core::{
    Decode, DecodeError, DecodeResult, Encode, EncodeResult, InvalidFieldErr as _, ReadCursor,
    WriteCursor, cast_length, decode_cursor, ensure_fixed_part_size, ensure_size,
    invalid_field_err,
};
...
impl<'de> Decode<'de> for FastPathHeader {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let (length, sizeof_length) = per::read_length(src).map_err(|e| {
            DecodeError::invalid_field("", "length", "Invalid encoded fast path PDU length", None)
                .with_source(e)
        })?;
    ...
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let (compression_flags, compression_type) =
            if compression.contains(Compression::COMPRESSION_USED) {
                ...
                .ok_or_else(
                    || invalid_field_err!("compressionFlags", "invalid compression type", in: src),
                )?;
...
impl<'a> FastPathUpdate<'a> {
    ...
    pub fn decode_cursor_with_code(
        ...
        match code {
            ...
            _ => {
                Err(invalid_field_err!("updateCode", "unsupported fast-path update code", in: src))
            }
        }
    }
