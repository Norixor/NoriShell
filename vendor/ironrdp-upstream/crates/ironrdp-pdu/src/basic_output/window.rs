...
fn validate_notification_icon_fields(flags: u32, src: &mut ReadCursor<'_>) -> DecodeResult<()> {
    ...
    const CACHED_ICON: u32 = 0x8000_0000;
    const ALLOWED: u32 =
        NOTIFY_ICON_TYPE | 0x0000_000F | 0x1000_0000 | DELETED | ICON | CACHED_ICON;
    if flags & !ALLOWED != 0 {
        return Err(invalid_field_err!(
            "fieldsPresent",
    ...
    if flags & 0x0000_0008 != 0 {
        ...
        if !matches!(src.read_u32(), 0 | 3 | 4) {
            return Err(invalid_field_err!(
                "Version",
                "unsupported notification icon version"
            ));
        }
    }
    ...
    if flags & 0x0000_0004 != 0 {
        ...
        if src.read_u32() & !1 != 0 {
            return Err(invalid_field_err!(
                "State",
                "unknown notification icon state"
            ));
        }
    }
    if flags & ICON != 0 {
...
fn validate_desktop_fields(flags: u32, src: &mut ReadCursor<'_>) -> DecodeResult<()> {
    ...
    if flags & 8 != 0 && flags & 2 == 0 {
        return Err(invalid_field_err!(
            "fieldsPresent",
            "desktop ARC began requires hooked"
        ));
    }
...
fn decode_orders<'a>(
    ...
    if !src.eof() {
        return Err(invalid_field_err!(
            "orderData",
            "orders update contains trailing data"
        ));
    }
