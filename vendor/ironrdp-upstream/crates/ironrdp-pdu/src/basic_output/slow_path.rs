...
use super::pointer::{
    CachedPointerAttribute, ColorPointerAttribute, LargePointerAttribute, PointerAttribute,
    PointerPositionAttribute, PointerUpdateData,
};
...
pub fn read_graphics_update_type(src: &mut ReadCursor<'_>) -> DecodeResult<GraphicsUpdateType> {
    ...
    match raw {
        ...
        _ => Err(invalid_field_err!( "updateType",
            "unknown slow-path graphics update type", in: src)),
    }
}
...
pub fn decode_slow_path_pointer<'a>(
    src: &mut ReadCursor<'a>,
) -> DecodeResult<PointerUpdateData<'a>> {
    ...
    match message_type {
        0x0001 => {
            ...
            match system_type {
                ...
                _ => Err(
                    invalid_field_err!("systemPointerType", "unknown system pointer type", in: src),
                ),
            }
        }
        0x0003 => {
        ...
        _ => Err(invalid_field_err!( "messageType",
            "unknown slow-path pointer message type", in: src)),
    }
}
