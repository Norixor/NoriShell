use ironrdp_core::{
    Decode, DecodeResult, Encode, EncodeResult, ReadCursor, WriteCursor, cast_length, decode,
...
pub use self::core_data::client::{
    ...
    ColorDepth, ConnectionType, HighColorDepth, IME_FILE_NAME_SIZE, KeyboardType,
...
pub use self::monitor_data::{
    ClientMonitorData, MONITOR_COUNT_MAX, MONITOR_COUNT_SIZE, MONITOR_FLAGS_SIZE, MONITOR_SIZE,
    Monitor, MonitorFlags,
};
...
pub use self::security_data::{
    ClientSecurityData, EncryptionLevel, EncryptionMethod, ServerSecurityData,
};
    ...
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        Ok(Self {
            core: core.ok_or_else(
                || invalid_field_err!("core", "required GCC core is absent", in: src),
            )?,
            security: security.ok_or_else(
                || invalid_field_err!("security", "required GCC security is absent", in: src),
            )?,
            network,
            cluster,
            monitor,
    ...
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        Ok(Self {
            ...
            network: network.ok_or_else(
                || invalid_field_err!("network", "required GCC network is absent", in: src),
            )?,
            security: security.ok_or_else(
                || invalid_field_err!("security", "required GCC security is absent", in: src),
            )?,
...
impl UserDataHeader {
        ...
        T: FromPrimitive,
    {
        ...
        let block_type = T::from_u16(raw_type)
            .ok_or_else(|| invalid_field_err!("blockType", "invalid GCC type", in: src))?;
