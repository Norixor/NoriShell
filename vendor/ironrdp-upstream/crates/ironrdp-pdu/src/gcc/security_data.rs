...
impl<'de> Decode<'de> for ServerSecurityData {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        let encryption_method = EncryptionMethod::from_bits(src.read_u32()).ok_or_else(
            || invalid_field_err!("encryptionMethod", "invalid encryption method", in: src),
        )?;
        let encryption_level = EncryptionLevel::from_u32(src.read_u32()).ok_or_else(
            || invalid_field_err!("encryptionLevel", "invalid encryption level", in: src),
        )?;

        let (server_random, server_cert) = if encryption_method.is_empty()
            && encryption_level == EncryptionLevel::None
        {
            (None, Vec::new())
        } else {
            ...
            let server_random_len: usize =
                cast_length!("serverRandomLen", src.read_u32(), in: src)?;
            if server_random_len != SERVER_RANDOM_LEN {
                return Err(
                    invalid_field_err!("serverRandomLen", "Invalid server random length", in: src),
                );
            }
            ...
            if server_cert_len > MAX_SERVER_CERT_LEN {
                return Err(
                    invalid_field_err!("serverCetLen", "Invalid server certificate length", in: src),
                );
            }
