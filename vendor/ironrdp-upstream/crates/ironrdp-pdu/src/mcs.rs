...
use ironrdp_core::{
    Decode, Encode, IntoOwned, ReadCursor, WriteBuf, WriteCursor, cast_length, decode, encode_buf,
    encode_vec, ensure_fixed_part_size, ensure_size, invalid_field_err, other_err, read_padding,
...
macro_rules! per_field_err {
    ($field_name:expr, in: $cursor:expr) => {{
        ...
        move |error| {
            ironrdp_core::invalid_field_err_with_source(
                Self::MCS_NAME,
                $field_name,
                "PER",
                offset,
...
impl<'de> McsPdu<'de> for AttachUserConfirm {
    ...
    fn mcs_body_encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        per::write_u16(dst, self.initiator_id, BASE_CHANNEL_ID)
            .map_err(per_field_err!("initiator", in: dst))?;
    ...
    fn mcs_body_decode(src: &mut ReadCursor<'de>, _: usize) -> DecodeResult<Self> {
        ...
        let result =
            per::read_enum(src, RESULT_ENUM_LENGTH).map_err(per_field_err!("result", in: src))?;
        let user_id =
            per::read_u16(src, BASE_CHANNEL_ID).map_err(per_field_err!("userId", in: src))?;
...
impl<'de> McsPdu<'de> for ChannelJoinRequest {
    ...
    fn mcs_body_decode(src: &mut ReadCursor<'de>, _: usize) -> DecodeResult<Self> {
        ...
        let initiator_id =
            per::read_u16(src, BASE_CHANNEL_ID).map_err(per_field_err!("initiator", in: src))?;
...
impl<'de> McsPdu<'de> for ChannelJoinConfirm {
    ...
    fn mcs_body_encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        per::write_u16(dst, self.requested_channel_id, 0)
            .map_err(per_field_err!("requested", in: dst))?;
        per::write_u16(dst, self.channel_id, 0).map_err(per_field_err!("channelId", in: dst))?;
    ...
    fn mcs_body_decode(src: &mut ReadCursor<'de>, _: usize) -> DecodeResult<Self> {
        ...
        let requested_channel_id =
            per::read_u16(src, 0).map_err(per_field_err!("requested", in: src))?;
        let channel_id = per::read_u16(src, 0).map_err(per_field_err!("channelId", in: src))?;
...
impl<'de> McsPdu<'de> for SendDataRequest<'de> {
    ...
    fn mcs_body_encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        per::write_length(
            dst,
            cast_length!("user-data-length", self.user_data.len(), in: dst)?,
        );
    ...
    fn mcs_body_decode(
        ...
        let (length, _) =
            per::read_length(src).map_err(per_field_err!("userDataLength", in: src))?;
...
impl<'de> McsPdu<'de> for SendDataIndication<'de> {
    ...
    fn mcs_body_encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        per::write_length(
            dst,
            cast_length!("userDataLength", self.user_data.len(), in: dst)?,
        );
        dst.write_slice(&self.user_data);
...
impl<'de> McsPdu<'de> for DisconnectProviderUltimatum {
    ...
    fn mcs_body_decode(src: &mut ReadCursor<'de>, _: usize) -> DecodeResult<Self> {
        ...
        Ok(Self {
            reason: DisconnectReason::from_u8(reason).ok_or_else(|| {
                invalid_field_err(Self::MCS_NAME, "reason", "unknown variant", Some(src.pos()))
            })?,
        })
    }
...
mod legacy {
    ...
    use super::{
        ConnectInitial, ConnectResponse, DomainParameters, RESULT_ENUM_LENGTH, ReadCursor,
        WriteCursor, cast_length, ensure_size,
    };
    ...
    impl<'de> Decode<'de> for ConnectResponse {
        fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
            ...
            let called_connect_id =
                cast_int!("called_connect_id", ber::read_integer(src)?, in: src)?;
