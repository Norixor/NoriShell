...
use ironrdp_core::{
    ReadCursor, WriteCursor, ensure_size, invalid_field_err, unexpected_message_type_err,
};
use tap::prelude::*;
...
impl OpaqueRoutingToken {
    pub fn read(src: &mut ReadCursor<'_>) -> DecodeResult<Option<Self>> {
        let start = src.pos();
        let Some(length) = src.inner()[start..]
            .windows(2)
...
impl ConnectionRequest {
    ...
    fn encode_after_nego_data(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        flags.set(
            RequestFlags::CORRELATION_INFO_PRESENT,
            self.correlation_info.is_some(),
        );
    ...
    fn decode_after_nego_data(
        ...
        if variable_part_rest_size > 0
            && variable_part_rest_size < usize::from(Self::RDP_NEG_REQ_SIZE)
        {
            return Err(invalid_field_err!(
                Self::NAME,
        ...
        if variable_part_rest_size >= usize::from(Self::RDP_NEG_REQ_SIZE) {
            ...
            if length != Self::RDP_NEG_REQ_SIZE {
                return Err(
                    invalid_field_err!(Self::NAME, "length", "must be eight bytes", in: src),
                );
            }
            ...
            let correlation_info = if flags.contains(RequestFlags::CORRELATION_INFO_PRESENT) {
                if variable_part_rest_size
                    != usize::from(Self::RDP_NEG_REQ_SIZE + CorrelationInfo::SIZE)
                {
                    return Err(invalid_field_err!(
                        Self::NAME,
                        "TPDU header variable part",
...
impl<'de> X224Pdu<'de> for ConnectionRequestWithOpaqueRoutingToken {
    ...
    fn x224_body_decode(
        ...
        let variable_part_rest_size = variable_part_size
            .checked_sub(routing_token.size())
            .ok_or_else(|| {
                invalid_field_err!(
                    Self::NAME,
                    "TPDU header variable part",
        ...
        let request =
            ConnectionRequest::decode_after_nego_data(src, variable_part_rest_size, None)?;
    ...
    fn x224_body_decode(
        ...
        if variable_part_size > 0 {
            ...
            match NegoMsgType::from(src.read_u8()) {
                ...
                unexpected => Err(
                    unexpected_message_type_err!(Self::X224_NAME, u8::from(unexpected), in: src),
                ),
            }
        } else {
            Ok(Self::Response {
...
fn read_nego_data(
    ...
    let data = core::str::from_utf8(&src.inner()[identifier_start..identifier_end])
        .map_err(|_| {
            invalid_field_err(ctx, "identifier", "not valid UTF-8", Some(identifier_start))
        })?
        .to_owned();
