...
use ironrdp_core::{
    Decode, DecodeResult, Encode, EncodeResult, IntoOwned, ReadCursor, WriteCursor, cast_int,
...
impl FileContentsFlags {
    ...
    pub fn validate(self) -> Result<(), &'static str> {
        ...
        match (size_set, range_set) {
            (true, true) => {
                Err("SIZE and RANGE flags are mutually exclusive per MS-RDPECLIP 2.2.5.3")
            }
...
impl<'a> FileContentsResponse<'a> {
    ...
    pub fn data_as_size(&self) -> DecodeResult<u64> {
        ...
        let chunk: [u8; 8] = self.data.as_ref().try_into().map_err(|_| {
            invalid_field_err!(
                "requestedFileContentsData",
                "SIZE response data is not 8 bytes"
            )
        })?;
...
impl Encode for FileContentsResponse<'_> {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        let header =
            PartialHeader::new_with_flags(cast_int!("dataLen", self.inner_size(), in: dst)?, flags);
        header.encode(dst)?;
...
impl<'de> Decode<'de> for FileContentsResponse<'de> {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        if header.data_length() < Self::FIXED_PART_SIZE {
            return Err(
                invalid_field_err!("requestedFileContentsData", "invalid data size", in: src),
            );
        };
