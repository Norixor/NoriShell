...
use pdu::{
    ...
    ClipboardFormatName, ClipboardGeneralCapabilityFlags, ClipboardPdu, ClipboardProtocolVersion,
    FileContentsFlags, FileContentsRequest, FileContentsResponse, FileDescriptor,
    FormatDataRequest, FormatListResponse, LockDataId, OwnedFormatDataResponse, PackedFileList,
};
...
impl<R: Role> Cliprdr<R> {
            ...
            FormatListResponse::Fail => {
                ...
                if !self.sent_file_contents_requests.is_empty() {
                    ...
                    let stream_ids: Vec<u32> =
                        self.sent_file_contents_requests.keys().copied().collect();
    ...
    fn drive_timeouts_impl(&mut self) -> PduResult<CliprdrSvcMessages<R>> {
        ...
        let inactivity_timeout_ms =
            u64::try_from(self.lock_inactivity_timeout.as_millis()).unwrap_or(u64::MAX);
        ...
        let transfer_timeout_ms =
            u64::try_from(self.transfer_timeout.as_millis()).unwrap_or(u64::MAX);
        ...
        if !stale_lock_ids.is_empty() {
            debug!(
                count = stale_lock_ids.len(),
                "Upload inactivity cleanup completed"
            );
        }
    ...
    pub fn request_file_contents(
        &mut self,
        mut request: FileContentsRequest,
    ) -> PduResult<CliprdrSvcMessages<R>> {
                ...
                if let Some(file_desc) = file_list.files.get(validated_file_index) {
                    if let Some(file_size) = file_desc.file_size {
                        let end_position = request
                            .position
                            .saturating_add(u64::from(request.requested_size));
        ...
        if self
            .sent_file_contents_requests
            .contains_key(&request.stream_id)
        {
            warn!(
                stream_id = request.stream_id,
    ...
    pub fn initiate_file_copy(
        ...
        self.local_file_list = Some(PackedFileList {
        ...
        let formats = vec![
            ...
            ClipboardFormat::new(drop_effect_id)
                .with_name(ClipboardFormatName::PREFERRED_DROP_EFFECT),
        ];
        ...
        let format_list = self
            .build_format_list(&formats)
            .map_err(|e| encode_err!(e))?;
        ...
        match pdu {
            ...
            ClipboardPdu::UnlockData(id) => {
                ...
                if self.locked_file_lists.remove(&id.0).is_some() {
                    ...
                    debug!(
                        clip_data_id = id.0,
                        "Received Unlock PDU but no locked file list found"
                    );
                }
            ...
            ClipboardPdu::FormatDataRequest(request) => {
                ...
                if Some(request.format) == self.local_drop_effect_format_id {
                    ...
                    let response =
                        OwnedFormatDataResponse::new_data(DROPEFFECT_COPY.to_le_bytes().to_vec());
                    let pdu = ClipboardPdu::FormatDataResponse(response);
                ...
                if Some(request.format) == self.local_file_list_format_id {
                    if let Some(ref file_list) = self.local_file_list {
                        ...
                        let response = OwnedFormatDataResponse::new_file_list(file_list)
                            .map_err(|e| encode_err!(e))?;
                        let pdu = ClipboardPdu::FormatDataResponse(response);
                        return Ok(vec![into_cliprdr_message(pdu)]);
                    } else {
                        ...
                        warn!(
                            "Received FormatDataRequest for file list format but no file list stored"
                        );
                    }
                }
            ...
            ClipboardPdu::FormatDataResponse(response) => {
                ...
                let is_file_list_response = requested_format.is_some()
                    && requested_format == self.remote_file_list_format_id;
                    ...
                    if response.is_error() {
                        ...
                        match response.to_file_list() {
                            Ok(mut file_list) => {
                                ...
                                self.backend
                                    .on_remote_file_list(&file_list.files, self.current_lock_id);
                            ...
                            Err(err) => {
                                error!(
                                    ?err,
                                    "Failed to parse FileGroupDescriptorW from FormatDataResponse"
                                );
                    ...
                    match self.locked_file_lists.get(&clip_data_id) {
                        Some(locked_list) => {
                            ...
                            debug!(
                                stream_id = request.stream_id,
                                clip_data_id,
                                "Using locked file list snapshot for FileContentsRequest"
                            );
                            Some(locked_list)
                        }
                        None => {
                            ...
                            debug!(
                                stream_id = request.stream_id,
                                clip_data_id,
                                "Locked file list snapshot expired, falling back to local file list"
                            );
            ...
            ClipboardPdu::FileContentsResponse(response) => {
                ...
                if let Some(transfer_state) = self.sent_file_contents_requests.remove(&stream_id) {
                    ...
                    if response.is_error() {
                        warn!(
                            stream_id,
                            "FileContentsResponse indicates failure (CB_RESPONSE_FAIL)"
                        );
                        ...
                        if !response.data().is_empty() {
                            ...
                            self.backend.on_file_contents_response(
                                FileContentsResponse::new_error(stream_id),
                            );
                        } else {
                            self.backend.on_file_contents_response(response);
                        }
                    } else if transfer_state.flags.contains(FileContentsFlags::SIZE)
                        && response.data().len() != 8
                    {
...
impl<R: Role> Cliprdr<R> {
    ...
    pub fn __test_sent_file_contents_requests_mut(
        &mut self,
    ) -> &mut HashMap<u32, FileTransferState> {
