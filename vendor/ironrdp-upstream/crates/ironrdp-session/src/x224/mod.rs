...
use ironrdp_pdu::rdp::headers::{
    BasicSecurityHeader, BasicSecurityHeaderFlags, CompressionFlags, IoChannelPdu, ShareDataCtx,
    ShareDataPdu,
};
...
use ironrdp_svc::{
    StaticChannelSet, SvcMessage, SvcProcessor, SvcProcessorMessages,
...
impl Processor {
    ...
    pub fn set_static_channel_chunk_size(&mut self, maximum_chunk_size: usize) -> bool {
        self.static_channels
            .set_maximum_chunk_size(maximum_chunk_size)
    }
                ...
                Err(error) => {
                        ...
                        decode::<X224<McsMessage<'_>>>(frame)
                    {
                        ...
                        return Ok(vec![ProcessorOutput::Disconnect(
                            DisconnectDescription::McsDisconnect(ultimatum.reason),
                        )]);
                    }
    ...
    fn process_io_channel(
        ...
        let io_channel =
            ironrdp_pdu::rdp::headers::decode_io_channel(data_ctx).map_err(SessionError::decode)?;
    ...
    fn process_share_data(
        ...
        let (pdu, compression_flags) = match pdu {
            ShareDataPdu::Compressed { pdu_type, data } => {
                ...
                (
                    ShareDataPdu::decode_with_type(&data, pdu_type)
        ...
        match pdu {
            ...
            ShareDataPdu::ArcStatusPdu(status) => {
                if status != [0; 4] {
                    return Err(reason_err!(
                        "IO channel",
                        "invalid auto-reconnect status PDU"
                    ));
                }
            ...
            ShareDataPdu::MonitorLayout(monitor_layout) => {
                Ok(vec![ProcessorOutput::MonitorLayout(
    ...
    fn decompress_share_data(
        ...
        let decompressor = bulk_decompressor.as_mut().ok_or_else(|| {
            reason_err!(
                "slow-path",
                "received compressed share data without a decompressor"
            )
        })?;
    ...
    fn process_message_channel(
        ...
        let security_header =
            BasicSecurityHeader::decode(&mut peek).map_err(SessionError::decode)?;
        let flags = security_header.flags.difference(
            BasicSecurityHeaderFlags::RESET_SEQNO | BasicSecurityHeaderFlags::IGNORE_SEQNO,
        );

        if flags == BasicSecurityHeaderFlags::HEARTBEAT {
            let heartbeat =
                decode::<HeartbeatPdu>(data_ctx.user_data).map_err(SessionError::decode)?;
            debug!(
        ...
        if flags == BasicSecurityHeaderFlags::TRANSPORT_REQ {
            let request = decode::<MultitransportRequestPdu>(data_ctx.user_data)
                .map_err(SessionError::decode)?;
            debug!(
...
mod tests {
    ...
    fn processor_decompresses_slow_path_share_data() {
        ...
        let (compressed_size, flags) = compressor
            .compress(&source)
            .expect("source should compress");
        assert_ne!(
            flags & flags::PACKET_COMPRESSED,
            0,
            "test data must be compressed"
        );
        ...
        let compression_flags = CompressionFlags::from_bits_retain(
            u8::try_from(flags & !flags::COMPRESSION_TYPE_MASK)
                .expect("bulk flags should fit in a byte"),
        );
    ...
    fn processor_encodes_multitransport_response_on_message_channel() {
        ...
        assert_eq!(
            decode::<MultitransportResponsePdu>(&request.user_data)
                .expect("decode multitransport response"),
            response
        );
    }
    ...
    fn processor_gracefully_disconnects_on_provider_ultimatum() {
        ...
        assert!(matches!(
            outputs.as_slice(),
            [ProcessorOutput::Disconnect(
                DisconnectDescription::McsDisconnect(DisconnectReason::ProviderInitiated)
            )]
        ));
    }
    ...
    fn processor_rejects_truncated_share_control_pdu_length() {
        ...
        assert!(
            error
                .to_string()
                .contains("truncated Share Control PDU length")
        );
    }
