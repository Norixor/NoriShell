...
impl FastPathBulkDecompressionFailure {
    fn new(attributes: FragmentAttributes, payload_length: usize, error: &BulkError) -> Self {
        let error_kind = match error {
            BulkError::UnsupportedCompressionType(_) => {
                BulkDecompressionErrorKind::UnsupportedCompressionType
            }
            BulkError::InvalidCompressedData(_) => {
                BulkDecompressionErrorKind::InvalidCompressedData
            }
            BulkError::OutputBufferTooSmall { .. } => {
                BulkDecompressionErrorKind::OutputBufferTooSmall
            }
...
impl Processor {
    ...
    pub fn process(
        ...
        while !input.is_empty() {
            let update_result =
                self.process_single_update(&mut input, image, output, bulk_decompressor)?;
    ...
    fn process_single_update(
        ...
        let update_pdu = match decode_cursor::<FastPathUpdatePdu<'_>>(input) {
            ...
            {
                let DecodeErrorKind::NotEnoughBytes {
                    received, expected, ..
                } = error.kind()
                else {
        ...
        let update_data = if attributes
            .compression_flags
            .is_some_and(|flags| !flags.is_empty())
        {
            decompressed_data =
                Self::decompress_fragment_data(update_pdu.data, attributes, bulk_decompressor)?;
    ...
    fn decompress_fragment_data(
        ...
        let decompressor = bulk_decompressor.as_mut().ok_or_else(|| {
            reason_err!(
                "FastPath",
                "received compression control flags without a decompressor"
            )
        })?;
        ...
        let decompressed =
            decompressor.decompress(data, bulk_flags).map_err(|error| {
                SessionError::new(
                    "FastPath bulk decompression",
                    SessionErrorKind::FastPathBulkDecompression(
                        FastPathBulkDecompressionFailure::new(attributes, data.len(), &error),
                    ),
                )
            })?;
    ...
    pub fn process_bitmap_update(
        ...
        for update in bitmap_update.rectangles {
            trace!("{update:?}");
            let Some(update_rectangle) =
                image.bitmap_destination(&update.rectangle, update.width, update.height)
            else {
                ...
                .contains(ironrdp_pdu::bitmap::Compression::BITMAP_COMPRESSION)
            {
                if update.bits_per_pixel == 32 {
                    ...
                    match ironrdp_graphics::rle::decompress(
                        ...
                        Ok(RlePixelFormat::Rgb16) => apply_bitmap(image.apply_rgb16_bitmap(
                            &buf,
                            &update_rectangle,
                            update.width,
                        ))?,
                        Ok(RlePixelFormat::Rgb15) => apply_bitmap(image.apply_rgb15_bitmap(
                            &buf,
                            &update_rectangle,
                            update.width,
                        ))?,
                        Ok(RlePixelFormat::Rgb24) => apply_bitmap(image.apply_bgr24_bitmap(
                            &buf,
                            &update_rectangle,
                            update.width,
                        ))?,
                ...
                if padded_row_bytes != row_bytes {
                    ...
                    match update.bits_per_pixel {
                        ...
                        15 => apply_bitmap(image.apply_rgb15_bitmap(
                            &buf,
                            &update_rectangle,
                            update.width,
                        ))?,
                        16 => apply_bitmap(image.apply_rgb16_bitmap(
                            &buf,
                            &update_rectangle,
                            update.width,
                        ))?,
                        24 => apply_bitmap(image.apply_bgr24_bitmap(
                            &buf,
                            &update_rectangle,
                            update.width,
                        ))?,
    ...
    pub fn process_pointer_update(
        ...
        match update {
            ...
            PointerUpdateData::Color(pointer) => {
                ...
                let decoded_pointer =
                    match DecodedPointer::decode_color_pointer_attribute(&pointer, bitmap_target) {
                        Ok(pointer) => Arc::new(pointer),
                        Err(error) => {
                            self.fallback_after_pointer_decode_failure(
                                image,
                                &mut processor_updates,
                                cache_index,
                                error,
                            )?;
                            return Ok(processor_updates);
            ...
            PointerUpdateData::Large(pointer) => {
                ...
                let decoded_pointer =
                    match DecodedPointer::decode_large_pointer_attribute_with_palette(
                        &pointer,
                        bitmap_target,
                        Some(self.palette.colors()),
                    ) {
    ...
    fn fallback_after_pointer_decode_failure(
        ...
        warn!(
            pointer_error = error_kind,
            "Ignoring unsupported pointer update"
        );
...
fn qoi_apply(
    ...
    let rectangle = match header.channels {
        qoi::Channels::Rgb => {
            image.apply_rgb24(&decoded, &destination, destination.width(), false)?
        }
...
impl ProcessorBuilder {
    pub fn build(self) -> Processor {
        Processor {
            ...
            marker_processor: FrameMarkerProcessor::new(
    ...
    fn append_data(&mut self, data: &[u8], attributes: FragmentAttributes) -> SessionResult<()> {
        if let Some(fragmented_update) = self.fragmented_data.as_mut() {
            if fragmented_update.attributes.update_code != attributes.update_code
                || fragmented_update.attributes.compression_flags.is_some()
                    != attributes.compression_flags.is_some()
            {
                return Err(reason_err!(
                    "FastPath",
                    "inconsistent fragmented update metadata"
                ));
            }
