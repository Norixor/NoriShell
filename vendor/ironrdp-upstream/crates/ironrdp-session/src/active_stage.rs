// Modified for NoriShell; see vendor/README.md for upstream provenance.
use std::sync::Arc;
...
use ironrdp_dvc::{
    DrdynvcClient, DvcClientProcessor, DvcMessageBatch, DynamicChannelMut, DynamicChannelRef,
};
...
use ironrdp_pdu::window::{
    WindowingOrdersUpdate, try_decode_fast_path_windowing_orders,
    try_decode_slow_path_windowing_orders,
};
...
fn new_bulk_decompressor(compression_type: Option<CompressionType>) -> Option<BulkCompressor> {
    compression_type
        .map(|compression_type| BulkCompressor::new(to_bulk_compression_type(compression_type)))
}
...
impl ActiveStage {
    ...
    pub fn process_fastpath_input(
        ...
        for event_chunk in events.chunks(FastPathInput::MAX_EVENTS) {
            ...
            let fastpath_input =
                FastPathInput::new(event_chunk.to_vec()).map_err(SessionError::decode)?;
    ...
    pub fn process(
        ...
        let (mut stage_outputs, processor_updates) = match action {
            Action::FastPath => {
                ...
                let processor_updates = self.fast_path_processor.process(
                    image,
                    frame,
                    &mut output,
                    &mut self.bulk_decompressor,
                )?;
                (
            ...
            Action::X224 => {
                let x224_outputs = self
                    .x224_processor
                    .process(frame, &mut self.bulk_decompressor)?;
                    ...
                    match output {
                        x224::ProcessorOutput::GraphicsUpdate(data) => {
                            ...
                            if let Some(windowing_orders) = windowing_orders {
                                stage_outputs
                                    .push(ActiveStageOutput::WindowingOrders(windowing_orders));
                            }
                        }
                        x224::ProcessorOutput::PointerUpdate(data) => {
                            let updates = process_slow_path_pointer(
                                &mut self.fast_path_processor,
                                image,
                                &data,
                            )?;
                ...
                let (region, damage_regions) = composite_graphics_updates(
                    image,
                    graphics_updates.into_iter().map(|u| (u.region, u.data)),
                )?;
    ...
    pub fn set_static_channel_chunk_size(&mut self, maximum_chunk_size: usize) -> bool {
        self.x224_processor
            .set_static_channel_chunk_size(maximum_chunk_size)
    }
    ...
    pub fn encode_multitransport_response(
        &self,
        response: &MultitransportResponsePdu,
    ...
    pub fn get_dvc_mut<T: DvcClientProcessor + 'static>(
        &mut self,
    ) -> Option<DynamicChannelMut<'_, T>> {
    ...
    pub fn display_control_ready(&mut self) -> Option<bool> {
        let Some(dvc) = self.get_dvc::<DisplayControlClient>() else {
            return self
                ...
                .and_then(|drdynvc| {
                    drdynvc
                        .has_registered_dvc::<DisplayControlClient>()
                        .then_some(false)
                });
        };
    ...
    pub fn process_dvc_tunnel(
        ...
        // A pure UDP EGFX EndFrame does not pass through process(Action::X224).
        // Drain the compositor here so the caller receives its graphics update now.
        let (output_reset, graphics_updates) = self
            .get_dvc_mut::<GraphicsPipelineClient>()
            .map(|mut gfx| {
                let gfx = gfx.processor_mut();
    ...
    pub fn prepare_resize(
        ...
        if let Some(dvc) = self.get_dvc::<DisplayControlClient>() {
            ...
            if !display_control.ready() {
                debug!(
                    "Could not encode a resize: Display Control capabilities have not been received"
                );
                return None;
            }
    ...
    pub fn prepare_rdpei_dismiss_hovering(
        &mut self,
        contact_id: u8,
    ) -> Option<SessionResult<DvcMessageBatch>> {
        if let Some(dvc) = self.get_dvc::<RdpeiClient>() {
    ...
    ///
    /// Returns an error when Soft-Sync routes the channel through a multitransport tunnel.
    pub fn encode_rdpei_dismiss_hovering(
        &mut self,
        contact_id: u8,
    ) -> Option<SessionResult<Vec<u8>>> {
...
impl TryFrom<x224::ProcessorOutput> for ActiveStageOutput {
    ...
    fn try_from(value: x224::ProcessorOutput) -> Result<Self, Self::Error> {
        match value {
            ...
            x224::ProcessorOutput::SaveSessionInfo { logon_complete } => {
                Ok(Self::SaveSessionInfo { logon_complete })
            }
            x224::ProcessorOutput::DeactivateAll => Ok(Self::DeactivateAll),
            x224::ProcessorOutput::MultitransportRequest(pdu) => {
                Ok(Self::MultitransportRequest(pdu))
            }
            x224::ProcessorOutput::AutoDetect(request) => Ok(Self::AutoDetect(request)),
            x224::ProcessorOutput::AutoReconnectCookie(cookie) => {
                Ok(Self::AutoReconnectCookie(cookie))
            }
            ...
            x224::ProcessorOutput::GraphicsUpdate(_) | x224::ProcessorOutput::PointerUpdate(_) => {
                Err(SessionError::general(
                    "slow-path graphics/pointer updates should be handled before this conversion",
                ))
            }
        }
    }
...
fn process_slow_path_graphics(
    ...
    let update_type =
        slow_path::read_graphics_update_type(&mut src).map_err(SessionError::decode)?;

    match update_type {
        GraphicsUpdateType::Bitmap => {
            let bitmap =
                slow_path::decode_slow_path_bitmap(&mut src).map_err(SessionError::decode)?;
            fast_path_processor
        ...
        GraphicsUpdateType::Orders => {
            ...
            let orders =
                try_decode_slow_path_windowing_orders(&mut src).map_err(SessionError::decode)?;
...
fn validate_windowing_orders_support(
    ...
    if window_support_level == WindowSupportLevel::SupportedEx
        || !orders
            .orders
            .iter()
            .any(|order| order.requires_extended_support())
    {
        return Ok(());
    }
...
mod tests {
    ...
    use ironrdp_dvc::pdu::{
        CreateRequestPdu, DataPdu, DrdynvcDataPdu, DrdynvcServerPdu, SoftSyncChannelList,
        SoftSyncRequestPdu,
    };
    use ironrdp_egfx::client::GraphicsPipelineHandler;
    use ironrdp_egfx::pdu::{
        Color, CreateSurfacePdu, EndFramePdu, GfxPdu, MapSurfaceToOutputPdu,
        PixelFormat as GfxPixelFormat, ResetGraphicsPdu, SolidFillPdu,
    };
    ...
    use ironrdp_rdpei::pdu::{
        PenEventPdu, RdpInputProtocolVersion, RdpeiPdu, ScReadyPdu, TouchEventPdu,
    };
    ...
    fn full_redraw_prefers_suppress_output_toggle_when_supported() {
        ...
        assert_eq!(
            stage
                .request_full_redraw(1024, 768, true, false)
                .unwrap()
                .len(),
            1
        );
        assert!(
            stage
                .request_full_redraw(1024, 768, false, false)
                .unwrap()
                .is_empty()
        );
    }
    ...
    fn fastpath_input_splits_at_event_limit() {
        ...
        let events = (0..=u8::MAX)
            .map(|code| {
                FastPathInputEvent::UnicodeKeyboardEvent(KeyboardFlags::empty(), u16::from(code))
            })
            .collect::<Vec<_>>();
    ...
    fn monitor_layout_is_forwarded_from_x224() {
        ...
        let output =
            ActiveStageOutput::try_from(x224::ProcessorOutput::MonitorLayout(monitors.clone()))
    ...
    fn slow_path_windowing_orders_require_negotiated_support() {
        ...
        let (_, orders) =
            process_slow_path_graphics(&mut processor, &mut image, None, &update).unwrap();
        assert!(orders.is_none());

        let (_, orders) = process_slow_path_graphics(
            &mut processor,
            &mut image,
            Some(WindowSupportLevel::Supported),
            &update,
        )
        .unwrap();
    ...
    fn fast_path_windowing_orders_are_normalized_for_forwarding() {
        ...
        let normalized =
            process_fast_path_windowing_orders(Some(WindowSupportLevel::Supported), &update)
                .unwrap()
                .unwrap();
        assert_eq!(
            normalized,
    ...
    fn prepared_dvc_batches_preserve_channel_and_unframed_messages() {
        ...
        let resize = stage
            .prepare_resize(1024, 768, None, None)
            .unwrap()
            .unwrap();
    ...
    fn active_stage_exposes_and_validates_soft_sync_routing() {
        ...
        let soft_sync = DrdynvcServerPdu::SoftSyncRequest(SoftSyncRequestPdu::new(vec![
            SoftSyncChannelList::new(SoftSyncTunnelType::RELIABLE_UDP, vec![1, 2]),
        ]));
        ...
        assert_eq!(
            stage.dvc_tunnel_for_channel(2),
        ...
        let rdpei_ready = encode_vec(&RdpeiPdu::ScReady(ScReadyPdu::new(
        ...
        let response = stage
            ...
            .unwrap();
        assert_prepared_batch(&response.0, 2);
        assert!(response.1.is_empty());
    }

    struct TestGraphicsHandler;
    impl GraphicsPipelineHandler for TestGraphicsHandler {}

    #[test]
    fn pure_udp_egfx_end_frame_synchronously_updates_image() {
        let mut stage = active_stage_with_ready_dvcs();
        stage.enable_reliable_udp_dvc_tunnel().unwrap();
        process_drdynvc_pdu(
            stage.get_svc_processor_mut::<DrdynvcClient>().unwrap(),
            DrdynvcServerPdu::SoftSyncRequest(SoftSyncRequestPdu::new(vec![
                SoftSyncChannelList::new(SoftSyncTunnelType::RELIABLE_UDP, vec![3]),
            ])),
        );
        ...
        for pdu in [
            ...
            GfxPdu::EndFrame(EndFramePdu { frame_id: 7 }),
        ] {
            gfx_bytes.extend(encode_vec(&pdu).unwrap());
        }
        let tunnel_data = encode_vec(&DrdynvcServerPdu::Data(DrdynvcDataPdu::Data(DataPdu::new(
            3,
            wrap_uncompressed(&gfx_bytes),
        ))))
        .unwrap();
        let mut image = DecodedImage::new(PixelFormat::RgbA32, 1, 1);
        let (response, outputs) = stage
            .process_dvc_tunnel(&mut image, SoftSyncTunnelType::RELIABLE_UDP, &tunnel_data)
            .unwrap();
        assert_prepared_batch(&response, 3); // EGFX FrameAcknowledge
        assert_eq!(image.width(), 2);
        assert_eq!(image.height(), 2);
        assert_eq!(&image.data()[..4], &[255, 0, 0, 255]);
        assert!(
            matches!(outputs.as_slice(), [ActiveStageOutput::GraphicsUpdate(region)] if region.left == 0 && region.top == 0 && region.right == 1 && region.bottom == 1)
        );
        assert_eq!(
            stage.take_damage_regions(),
    ...
    fn active_stage_with_ready_dvcs() -> ActiveStage {
        ...
        process_drdynvc_pdu(
            ...
            DrdynvcServerPdu::Create(CreateRequestPdu::new(
                2,
                ironrdp_rdpei::CHANNEL_NAME.to_owned(),
            )),
        );
        ...
        let display_caps =
            DisplayControlPdu::Caps(DisplayControlCapabilities::new(1, 3840, 2400).unwrap());
        process_drdynvc_pdu(
            &mut drdynvc,
        ...
        process_drdynvc_pdu(
            ...
            DrdynvcServerPdu::Data(DrdynvcDataPdu::Data(DataPdu::new(
                2,
                encode_vec(&RdpeiPdu::ScReady(ScReadyPdu::new(
                    RdpInputProtocolVersion::V200,
                )))
                .unwrap(),
            ))),
