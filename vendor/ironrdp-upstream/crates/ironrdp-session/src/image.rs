...
impl DecodedImage {
    ...
    pub(crate) fn reset_preserving_pointer(
        &mut self,
        width: u16,
        height: u16,
    ) -> SessionResult<()> {
        // Keep the protocol framebuffer within the application's frame envelope before allocating.
        // This applies to EGFX carried by either TCP or UDP.
            ...
            || pixels > MAX_PIXELS
        {
            return Err(SessionError::general(
                "reset graphics framebuffer exceeds application limit",
            ));
        }
        let len = usize::from(width)
            ...
            .ok_or_else(|| {
                SessionError::general("reset graphics framebuffer dimensions overflow")
            })?;
    ...
    pub(crate) fn bitmap_destination(
        ...
        (self.rect_fits(update_rectangle)
            ...
            && update_rectangle.height() == height)
...
mod tests {
    ...
    #[test]
    fn reset_rejects_oversized_egfx_before_allocating_or_changing_image() {
        let mut image = DecodedImage::new(PixelFormat::RgbA32, 2, 2);
        let original_capacity = image.data.capacity();
        for (width, height) in [(8_193, 1), (8_192, 8_192), (0, 1)] {
            assert!(image.reset_preserving_pointer(width, height).is_err());
            assert_eq!((image.width(), image.height()), (2, 2));
            assert_eq!(image.data.capacity(), original_capacity);
        }
        image.reset_preserving_pointer(4_096, 4_096).unwrap();
        assert_eq!((image.width(), image.height()), (4_096, 4_096));
    }
    ...
    fn bitmap_application_rejects_invalid_rectangles_and_source_lengths() {
        ...
        assert_eq!(
            image
                .apply_rgb16_bitmap(&[], &invalid_rectangle, 1)
                .unwrap(),
        ...
        assert!(
            image
                .apply_rgb16_bitmap(&[0; 6], &encoded_rectangle, 1)
                .is_err(),
