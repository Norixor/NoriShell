...
impl Update {
    ...
    fn install_inner(&self, bytes: &[u8]) -> Result<()> {
        ...
        // Both atomic exchange and rollback require the app and staging areas
        // to live on the same file system.
        let device = self.extract_path.metadata()?.dev();
        if device != tmp_extract_dir.path().metadata()?.dev()
            || device != tmp_backup_dir.path().metadata()?.dev()
        {
        ...
        // The extraction directory becomes the bundle root after replacement.
        ...
        let need_authorization = match moved {
            ...
            Err(err) => {
                if backup.exists() {
                    // If rollback itself failed, retain the only copy of the old
                    // app instead of letting TempDir delete it on drop.
                    let _ = tmp_backup_dir.keep();
                    return Err(err.into());
                }
        ...
        if need_authorization {
            ...
            // Keep the old bundle until the replacement succeeds, including in
            // the privileged path. Quote paths for both shell and AppleScript.
            let shell = format!(
                "old={}; new={}; backup={}; mv -f \"$old\" \"$backup\" && {{ if mv -f \"$new\" \"$old\"; then rm -rf \"$backup\"; else mv -f \"$backup\" \"$old\"; exit 1; fi; }}",
                shell_quote_path(&self.extract_path)?,
                shell_quote_path(tmp_extract_dir.path())?,
            ...
            let res = (self.context.run_on_main_thread)(Box::new(move || {
                ...
                let succeeded = script.compile().is_ok() && script.execute().is_ok();
                let _ = tx.send(succeeded);
            }));
            let succeeded = rx.recv().unwrap_or(false);

            if res.is_err() || !succeeded {
                if backup.exists() {
                    let _ = tmp_backup_dir.keep();
                }
...
fn swap_bundle(target: &Path, staged: &Path) -> std::io::Result<()> {
    ...
    // SAFETY: both pointers are valid NUL-terminated paths for this call.
...
#[cfg(target_os = "macos")]
fn shell_quote_path(path: &Path) -> std::io::Result<String> {
    let value = path.to_str().ok_or_else(|| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Updater path is not UTF-8",
        )
    })?;
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn macos_swap_keeps_an_app_at_the_install_path() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("NoriShell.app");
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn macos_fallback_restores_the_old_app_after_a_failed_replacement() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("NoriShell.app");
        let backup = dir.path().join("old.app");
        std::fs::create_dir(&target).unwrap();
        std::fs::write(target.join("version"), "old").unwrap();
        let missing_staged = dir.path().join("missing.app");
        let error = super::replace_bundle(&target, &missing_staged, &backup).unwrap_err();
