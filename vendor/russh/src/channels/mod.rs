...
impl<S: From<(ChannelId, ChannelMsg)> + Send + Sync + 'static> Channel<S> {
    ...
    /// A write-only view for closing a channel when its setup future is cancelled.
    pub fn write_handle(&self) -> ChannelWriteHalf<S> {
        ChannelWriteHalf { id: self.write_half.id, sender: self.write_half.sender.clone(), max_packet_size: self.write_half.max_packet_size, window_size: self.write_half.window_size.clone() }
    }
