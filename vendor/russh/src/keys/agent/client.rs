...
impl<S: AgentStream + Unpin> AgentClient<S> {
    ...
    pub async fn request_identities(&mut self) -> Result<Vec<AgentIdentity>, Error> {
        ...
        self.read_response().await?;
        debug!("agent identities response received: {} bytes", self.buf.len());
    ...
    async fn sign_request_pk(
        ...
        debug!("agent sign request prepared: {} bytes", data.len());
        ...
        match self.buf.split_first() {
            ...
            _ => {
                debug!("agent returned an unexpected {}-byte response", self.buf.len());
    ...
    async fn sign_request_cert(
        ...
        debug!("agent certificate sign request prepared: {} bytes", data.len());
    ...
    fn prepare_sign_request(
        ...
        data.encode(&mut self.buf)?;
        debug!("agent sign request key algorithm: {}", public.algorithm());
