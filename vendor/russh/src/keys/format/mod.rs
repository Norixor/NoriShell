...
pub enum Encryption {
    ...
    /// Key for AES256
    Aes256Cbc([u8; 16]),
    /// Legacy three-key 3DES-CBC PEM encryption IV.
    TripleDesCbc([u8; 8]),
}
        ...
        for l in secret.lines() {
            if started {
                ...
                if l.chars().all(is_base64_char) {
                    sec.push_str(l)
                } else if l.starts_with("DEK-Info:") {
                    format = Some(Format::Pkcs5Encrypted(parse_legacy_pem_encryption(l)?))
                }
            }
...
fn parse_legacy_pem_encryption(line: &str) -> Result<Encryption, Error> {
    let (_, parameters) = line.split_once(':').ok_or(Error::CouldNotReadKey)?;
    let (algorithm, encoded_iv) = parameters
        .trim()
        .split_once(',')
        .ok_or(Error::CouldNotReadKey)?;
    let decoded_iv = HEXLOWER_PERMISSIVE.decode(encoded_iv.trim().as_bytes())?;
    match algorithm.trim() {
        "AES-128-CBC" | "AES-256-CBC" if decoded_iv.len() == 16 => {
            let mut iv = [0; 16];
            iv.copy_from_slice(&decoded_iv);
            if algorithm.trim() == "AES-128-CBC" {
                Ok(Encryption::Aes128Cbc(iv))
            } else {
                Ok(Encryption::Aes256Cbc(iv))
            }
        }
        "DES-EDE3-CBC" if decoded_iv.len() == 8 => {
            let mut iv = [0; 8];
            iv.copy_from_slice(&decoded_iv);
            Ok(Encryption::TripleDesCbc(iv))
        }
