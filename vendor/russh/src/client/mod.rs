...
/// Cloneable, request/reply-only view of one SSH client session.
///
/// It deliberately exposes no authentication or channel operations. A caller
/// may use it to schedule protocol keepalives while the owning transport keeps
/// the full [`Handle`] and session lifecycle.
#[derive(Clone)]
...
impl PingHandle {
    /// Send a keepalive global request and wait for either success or failure
    /// reply. Both are valid liveness replies; a closed session is not.
...
/// Cloneable channel capability for one already authenticated connection.
/// It cannot authenticate, rekey, or disconnect the owning connection.
#[derive(Clone)]
...
impl<H: Handler> Handle<H> {
    /// Returns a restricted view that opens channels on this same connection.
    pub fn session_channel_handle(&self) -> SessionChannelHandle {
        SessionChannelHandle { sender: self.sender.clone(), channel_buffer_size: self.channel_buffer_size }
    }
    ...
    /// Return a least-privilege handle for protocol keepalive requests.
    pub fn ping_handle(&self) -> PingHandle {
        PingHandle {
            sender: self.sender.clone(),
        }
    }
    ...
    pub async fn send_ping(&self) -> Result<(), Error> {
        self.ping_handle().send_ping().await
    }
