...
pub(crate) trait Select {
    ...
    fn read_kex(
        ...
        let client_cipher = Self::select(
            &pref.cipher,
            &NameList::decode(&mut r)?,
            AlgorithmKind::Cipher,
        )?
        .1;
        let server_cipher = Self::select(
            &pref.cipher,
            &NameList::decode(&mut r)?,
            AlgorithmKind::Cipher,
        )?
        ...
        let select_mac = |list: &[String], needs_mac| match Self::select(&pref.mac, list, AlgorithmKind::Mac) {
            Ok((_, m)) if needs_mac && m == mac::NONE => Err(Error::NoCommonAlgo {
        ...
        let client_mac = select_mac(&NameList::decode(&mut r)?, client_needs_mac)?;
        let server_mac = select_mac(&NameList::decode(&mut r)?, server_needs_mac)?;
...
mod tests {
    ...
    /// SSH negotiates encryption independently in each direction. Keeping
    /// both results is required both for key sizes and for accurate session
    /// facts when a peer deliberately offers asymmetric lists.
    #[test]
    fn asymmetric_directional_ciphers_are_preserved() {
        let pref = Preferred::DEFAULT;
        let mut buf = vec![msg::KEXINIT];
        ...
        names(pref.key.iter().map(ToString::to_string).collect())
            .encode(&mut buf)
            .unwrap();
        names(vec![cipher::AES_256_CTR.as_ref().to_string()])
            .encode(&mut buf)
            .unwrap();
        names(vec![cipher::AES_128_CTR.as_ref().to_string()])
            .encode(&mut buf)
            .unwrap();
        for _ in 0..2 {
            names(pref.mac.iter().map(|x| x.as_ref().to_string()).collect())
        ...
        Vec::<String>::new().encode(&mut buf).unwrap();
        Vec::<String>::new().encode(&mut buf).unwrap();
        0u8.encode(&mut buf).unwrap();
        0u32.encode(&mut buf).unwrap();

        let negotiated =
            Server::read_kex(&buf, &pref, None, None, &KexCause::Initial).unwrap();
        assert_eq!(negotiated.client_cipher, cipher::AES_256_CTR);
        assert_eq!(negotiated.server_cipher, cipher::AES_128_CTR);
    }
