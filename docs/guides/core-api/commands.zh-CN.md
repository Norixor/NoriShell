...
`desktop_preferences_get` / `desktop_preferences_replace` 只供可信主窗口读取与 CAS 替换非秘密桌面偏好；revision 使用十进制字符串。

`application_preferences_get` / `application_preferences_replace` 只供可信主窗口逐组读取与 CAS 替换六组非秘密应用偏好。首次迁移前 `revision` 与 `value` 均为 null；替换须提供预期修订号，Core 校验字段。插件不能调用这两个 IPC command；需要独立 `appPreferencesRead` 授权才能通过 Broker 读取八组快照。

`tray_actions_ready` 在注册 `native-tray-action` listener 后调用，返回待消费 token；`tray_action_take` 消费绑定原资源代次的一次性 token，不接受 renderer 提交目标。新建动作仍经过原 Launcher/认证门禁；资源定位绝不重连。
...
分类数据操作属于 `PluginApiOperation` Broker variant，不新增 renderer Tauri command；见 [Plugin Host 边界](plugin-host.zh-CN.md)与[插件 API](../plugin-api/README.zh-CN.md)。
