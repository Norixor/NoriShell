...
This is the maintained index for application IPC. Each stable command's exact `RequestType → ResultType` comes from [`core-api.ts`](../../../src/core-api/generated/core-api.ts). The complete stable list is [the generated command directory](commands.generated.md): 219 symbols from `COMMAND_*` declarations. The actual production registry is separately enumerated in [the handler and ACL directory](handlers.generated.md): 275 handlers from `production_invoke_handler!` and the trusted main-window allowlist.
...
`desktop_preferences_get` / `desktop_preferences_replace` read and CAS-replace non-secret preferences from the trusted main window; revisions are decimal strings.

`application_preferences_get` / `application_preferences_replace` let the trusted main window read and CAS-replace six non-secret application preference groups. Before first migration, `revision` and `value` are null. Replacement supplies the expected revision and Core validates the group fields. Plugins cannot invoke these IPC commands; Broker reads of eight groups require a separate `appPreferencesRead` grant.

Call `tray_actions_ready` after registering the `native-tray-action` listener, then consume each immutable token with `tray_action_take`. The renderer cannot submit a replacement target. Explicit creation follows the existing launcher/authentication gates; focusing never reconnects.
...
Category data operations are `PluginApiOperation` broker variants, not new renderer Tauri commands; see [plugin-host bridge](plugin-host.en.md) and [Plugin API](../plugin-api/README.en.md).
