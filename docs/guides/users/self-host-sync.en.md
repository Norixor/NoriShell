...
The previous Core-owned automatic scheduler is being replaced by plugin-owned scheduling. The new automatic sync behavior is not yet validated; use explicit sync until the new candidate has passed native acceptance.

When the page says review is needed without a specific error, select Review and sync. The plugin's “Use local/remote for conflicts” chooses the content of conflicting items. Core may then separately ask whether to apply the verified result on this device and say whether the cloud was already written. Cancelling local application does not undo an upload that has completed. When a specific error appears, follow that message. Local ownership, sync key binding, and object restore conflicts never overwrite data automatically.
