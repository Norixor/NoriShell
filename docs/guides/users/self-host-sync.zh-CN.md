...
当前版本尚未接入自动同步调度；请在同步页明确点击“立即同步”。

页面显示“需要审阅”且没有具体错误时，点击“审阅并同步”继续。Core 安全窗口展示本机与云端两种完整结果，可在确认写入本机前选择冲突项目来源，不会为相同选择再次确认。云端上传若已完成，取消本机写入不会撤销它。若显示具体错误，则按错误说明处理；本机数据归属、同步密钥绑定或恢复对象冲突不会自动覆盖数据。
